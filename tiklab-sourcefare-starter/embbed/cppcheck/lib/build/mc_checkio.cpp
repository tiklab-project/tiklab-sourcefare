...
MAYBE_UNUSED static inline bool match7(const Token* tok) {
    if (!tok || !(tok->str() == MatchCompiler::makeConstString("fopen")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")))
        return false;
...
MAYBE_UNUSED static inline bool match44(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("const")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("struct"))))
        return false;
    return true;
}
// pattern: %any% ::
...
#line 1 "/opt/cppcheck/cppcheck-main/lib/checkio.cpp"
/*
...
void CheckIO::checkFileUsage()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        ...
        for (const Token *tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            if (match4(tok) && isUnevaluated(tok)) {
                tok = tok->linkAt(1);
                continue;
            }
            if (tok->str() == MatchCompiler::makeConstString("{"))
            ...
            } else if (match4(tok) && tok->previous() && (!tok->previous()->isName() || match6(tok->previous()))) {
                std::string mode;
                ...
                } else if (!match9(tok) && !mSettings->library.isFunctionConst(tok->str(), true)) {
                ...
                case Filepointer::Operation::POSITIONING:
                    ...
                    else if (f.append_mode == Filepointer::AppendMode::APPEND && tok->str() != MatchCompiler::makeConstString("fflush") && printWarnings)
                       ...
                       const Token *&formatStringTok, const Token *&formatArgTok)
{
    ...
    if (match14(argTok) &&
        argTok->variable() &&
        match15(argTok->variable()->typeStartToken()) &&
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token *tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (formatStringArgNo >= 0) {
                ...
                if (match17(tok->tokAt(2)->nextArgument())) {
                                ...
                                const bool scanf_s)
{
                        ...
                        while (!done) {
                            ...
                            case 'i':
                                ...
                                if (argInfo.typeToken->tokType() == Token::eString) {
                                    ...
                                    if (argInfo.typeToken->isUnsigned() && !match24(argInfo.typeToken)) {
                                        if (!(!argInfo.isArrayOrPointer() && argInfo.element))
                                            invalidPrintfArgTypeError_sint(tok, numFormat, specifier, &argInfo);
                                    } else if (!match25(argInfo.typeToken)) {
                                        if (!(!argInfo.isArrayOrPointer() && argInfo.element))
                                            invalidPrintfArgTypeError_sint(tok, numFormat, specifier, &argInfo);
                                    } else {
                                        switch (specifier[0]) {
                            ...
                            case 'u':
                                ...
                                if (argInfo.typeToken->tokType() == Token::eString) {
                                    ...
                                    if (!argInfo.typeToken->isUnsigned() && !match27(argInfo.typeToken)) {
                                        if (!(!argInfo.isArrayOrPointer() && argInfo.element))
                                            invalidPrintfArgTypeError_uint(tok, numFormat, specifier, &argInfo);
                                    } else if (!match28(argInfo.typeToken)) {
                                        if (!(!argInfo.isArrayOrPointer() && argInfo.element))
                                            invalidPrintfArgTypeError_uint(tok, numFormat, specifier, &argInfo);
                                    } else {
                                        switch (specifier[0]) {
    ...
    int numFunction = 0;
    while (argListTok2) {
        if (match29(argListTok2)) // bailout for parameter pack
            return;
        numFunction++;
    ...
    : isCPP(_isCPP)
{
    ...
    if (!match30(arg) && !(arg->variable() && arg->variable()->isArray())) {
        const Token *top = arg;
        while (top->str() == MatchCompiler::makeConstString("(") && !top->isCast())
            top = top->next();
    ...
    if (arg->str() == MatchCompiler::makeConstString("&") || arg->tokType() == Token::eVariable ||
        arg->tokType() == Token::eFunction || match31(arg) ||
        (match32(arg) &&
         match33(arg->linkAt(1)) &&
         match34(arg->linkAt(1)->linkAt(1)))) {
        if (match35(arg)) {
        ...
        for (; tok1; tok1 = tok1->next()) {
            if (tok1->str() == MatchCompiler::makeConstString(",") || tok1->str() == MatchCompiler::makeConstString(")")) {
                ...
                } else if (tok1->strAt(-1) == MatchCompiler::makeConstString(")") && tok1->linkAt(-1)->previous()->tokType() == Token::eFunction) {
            ...
            if (tok1->str() == MatchCompiler::makeConstString("(") || tok1->str() == MatchCompiler::makeConstString("{") || tok1->str() == MatchCompiler::makeConstString("["))
                tok1 = tok1->link();
            else if (tok1->link() && tok1->str() == MatchCompiler::makeConstString("<"))
                tok1 = tok1->link();

            // check for some common well known functions
            else if (isCPP && ((match36(tok1->previous()) && isStdContainer(tok1->previous())) ||
                               (match37(tok1->previous()) && isStdContainer(tok1->linkAt(-1)->previous())))) {
                tempToken = new Token(tok1);
                if (tok1->strAt(1) == MatchCompiler::makeConstString("size")) {
                    ...
                    tempToken->isUnsigned(true);
                } else if (tok1->strAt(1) == MatchCompiler::makeConstString("empty")) {
                    tempToken->str("bool");
                } else if (tok1->strAt(1) == MatchCompiler::makeConstString("c_str")) {
                    tempToken->str("const");
                    tempToken->insertToken("*");
            ...
            else if (match38(tok1->previous()) &&
                     match39(tok1->linkAt(2))) {
                varTok = tok1->previous();
        ...
        if (varTok) {
            variableInfo = varTok->variable();
            element = tok1->strAt(-1) == MatchCompiler::makeConstString("]");
...
bool CheckIO::ArgumentInfo::isArrayOrPointer() const
{
    ...
    while (match44(tok))
        tok = tok->next();
    return tok && tok->strAt(1) == MatchCompiler::makeConstString("*");
}
...
bool CheckIO::ArgumentInfo::isKnownType() const
{
    ...
    if (functionInfo)
        return (typeToken->isStandardType() || functionInfo->retType || match41(typeToken));

    return typeToken->isStandardType() || match41(typeToken);
}
...
void CheckIO::invalidScanfFormatWidthError(const Token* tok, nonneg int numFormat, int width, const Variable *var, const std::string& specifier)
{
    ...
    if (arrlen > width) {
        ...
        errmsg << "Width " << width << " given in format string (no. " << numFormat << ") is larger than destination buffer '"
               << varname << "[" << arrlen << "]', use %" << (specifier == MatchCompiler::makeConstString("c") ? arrlen : (arrlen - 1)) << specifier << " to prevent overflowing it.";
