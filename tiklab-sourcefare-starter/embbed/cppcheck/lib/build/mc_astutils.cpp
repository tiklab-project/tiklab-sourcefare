...
// pattern: std :: string|wstring|u16string|u32string !!::
...
MAYBE_UNUSED static inline bool match14(const Token* tok) {
    if (!tok || !((tok->str() == MatchCompiler::makeConstString(".")) || ((tok->tokType() == Token::eExtendedOp || tok->tokType() == Token::eLambda) && tok->str() == MatchCompiler::makeConstString("[")) || ((tok->tokType() == Token::eIncDecOp) && tok->str() == MatchCompiler::makeConstString("++")) || ((tok->tokType() == Token::eIncDecOp) && tok->str() == MatchCompiler::makeConstString("--")) || tok->isName() || tok->isAssignmentOp()))
        return false;
    return true;
}
// pattern: &|<<|>>
MAYBE_UNUSED static inline bool match15(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eBitOp) && tok->str() == MatchCompiler::makeConstString("&")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("<<")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString(">>"))))
        return false;
    return true;
}
// pattern: ?
...
MAYBE_UNUSED static inline bool match18(const Token* tok) {
    if (!tok || !(tok->str() == MatchCompiler::makeConstString("typeid")))
        return false;
    return true;
}
// pattern: &
...
// pattern: . push_back|push_front|insert|push (
MAYBE_UNUSED static inline bool match35(const Token* tok) {
    if (!tok || !(tok->str() == MatchCompiler::makeConstString(".")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->str() == MatchCompiler::makeConstString("push_back")) || (tok->str() == MatchCompiler::makeConstString("push_front")) || (tok->str() == MatchCompiler::makeConstString("insert")) || (tok->str() == MatchCompiler::makeConstString("push"))))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")))
        return false;
...
// pattern: return|(|{|%assign%
MAYBE_UNUSED static inline bool match36(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("return")) || ((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")) || ((tok->tokType() == Token::eBracket) && tok->str() == MatchCompiler::makeConstString("{")) || tok->isAssignmentOp()))
        return false;
    return true;
}
// pattern: do {
...
// pattern: ; %varid% =
MAYBE_UNUSED static inline bool match48(const Token* tok, const int varid) {
    if (!tok || !(tok->str() == MatchCompiler::makeConstString(";")))
        return false;
    tok = tok->next();
    if (varid==0U)
...
MAYBE_UNUSED static inline bool match57(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eLogicalOp) && tok->str() == MatchCompiler::makeConstString("!")) || ((tok->tokType() == Token::eLogicalOp) && tok->str() == MatchCompiler::makeConstString("&&")) || (tok->tokType() == Token::eLogicalOp && tok->str() == MatchCompiler::makeConstString("||")) || tok->isComparisonOp()))
        return false;
    return true;
}
// pattern: &&|!|%oror%
MAYBE_UNUSED static inline bool match58(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eLogicalOp) && tok->str() == MatchCompiler::makeConstString("&&")) || ((tok->tokType() == Token::eLogicalOp) && tok->str() == MatchCompiler::makeConstString("!")) || (tok->tokType() == Token::eLogicalOp && tok->str() == MatchCompiler::makeConstString("||"))))
        return false;
    return true;
}
...
MAYBE_UNUSED static inline bool match70(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")) || (tok->str() == MatchCompiler::makeConstString(".")) || ((tok->tokType() == Token::eExtendedOp || tok->tokType() == Token::eLambda) && tok->str() == MatchCompiler::makeConstString("["))))
        return false;
    return true;
}
// pattern: %var% . %name% (
...
// pattern: get|get_deleter ( )
MAYBE_UNUSED static inline bool match74(const Token* tok) {
    if (!tok || !((tok->str() == MatchCompiler::makeConstString("get")) || (tok->str() == MatchCompiler::makeConstString("get_deleter"))))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")))
        return false;
...
MAYBE_UNUSED static inline bool match77(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("return")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("goto")) || (tok->str() == MatchCompiler::makeConstString("throw")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("continue")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("break"))))
        return false;
    return true;
}
// pattern: exit|abort
MAYBE_UNUSED static inline bool match78(const Token* tok) {
    if (!tok || !((tok->str() == MatchCompiler::makeConstString("exit")) || (tok->str() == MatchCompiler::makeConstString("abort"))))
        return false;
    return true;
}
// pattern: ; ;
...
// pattern: break
template<class T> MAYBE_UNUSED static inline T * findmatch83(T * start_tok, const Token * end) {
...
MAYBE_UNUSED static inline bool match96(const Token* tok) {
    if (!tok || !(tok->isName() || ((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")) || ((tok->tokType() == Token::eBracket) && tok->str() == MatchCompiler::makeConstString("{"))))
        return false;
    return true;
}
// pattern: %name% :
...
MAYBE_UNUSED static inline bool match99(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString(",")) || (tok->str() == MatchCompiler::makeConstString(";")) || ((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString(":")) || ((tok->tokType() == Token::eExtendedOp || tok->tokType() == Token::eLambda) && tok->str() == MatchCompiler::makeConstString("]")) || ((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString(")")) || ((tok->tokType() == Token::eBracket) && tok->str() == MatchCompiler::makeConstString("}"))))
        return false;
    return true;
}
// pattern: [ ]
...
MAYBE_UNUSED static inline bool match103(const Token* tok) {
    ...
    if (!tok || !(tok->str() == MatchCompiler::makeConstString("::")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str() == MatchCompiler::makeConstString("tie")))
        return false;
    return true;
}
// pattern: ,|(
...
// pattern: goto|return|throw
MAYBE_UNUSED static inline bool match118(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("goto")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("return")) || (tok->str() == MatchCompiler::makeConstString("throw"))))
        return false;
    return true;
}
...
MAYBE_UNUSED static inline bool match127(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("*")) || tok->isName() || (tok->str() == MatchCompiler::makeConstString("::")) || ((tok->tokType() == Token::eBracket || tok->tokType() == Token::eComparisonOp) && tok->str() == MatchCompiler::makeConstString(">"))))
        return false;
    return true;
}
// pattern: ] (|{|<
...
MAYBE_UNUSED static inline bool match131(const Token* tok) {
    if (!tok || !((tok->tokType() == Token::eLogicalOp && tok->str() == MatchCompiler::makeConstString("||")) || ((tok->tokType() == Token::eLogicalOp) && tok->str() == MatchCompiler::makeConstString("&&")) || ((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")) || ((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString(",")) || (tok->str() == MatchCompiler::makeConstString(".")) || ((tok->tokType() == Token::eLogicalOp) && tok->str() == MatchCompiler::makeConstString("!")) || (tok->str() == MatchCompiler::makeConstString(";")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("return"))))
        return false;
    return true;
}
// pattern: sizeof (
...
MAYBE_UNUSED static inline bool match134(const Token* tok) {
    if (!tok || !(tok->isConstOp() || ((tok->tokType() == Token::eExtendedOp || tok->tokType() == Token::eLambda) && tok->str() == MatchCompiler::makeConstString("[")) || (tok->str() == MatchCompiler::makeConstString("."))))
        return false;
    return true;
}
...
MAYBE_UNUSED static inline bool match136(const Token* tok) {
    if (!tok || !(tok->isComparisonOp() || ((tok->tokType() == Token::eLogicalOp) && tok->str() == MatchCompiler::makeConstString("!"))))
        return false;
    return true;
}
// pattern: ++|--
...
MAYBE_UNUSED static inline bool match138(const Token* tok) {
    if (!tok || !(tok->isConstOp() || tok->isAssignmentOp() || ((tok->tokType() == Token::eIncDecOp) && tok->str() == MatchCompiler::makeConstString("++")) || ((tok->tokType() == Token::eIncDecOp) && tok->str() == MatchCompiler::makeConstString("--"))))
        return false;
    return true;
}
// pattern: %assign%|++|--
...
MAYBE_UNUSED static inline bool match141(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("*")) || ((tok->tokType() == Token::eBitOp) && tok->str() == MatchCompiler::makeConstString("&")) || ((tok->tokType() == Token::eLogicalOp) && tok->str() == MatchCompiler::makeConstString("&&")) || ((tok->tokType() == Token::eExtendedOp || tok->tokType() == Token::eLambda) && tok->str() == MatchCompiler::makeConstString("["))))
        return false;
    return true;
}
// pattern: this . %var%
...
MAYBE_UNUSED static inline bool match143(const Token* tok) {
    if (!tok || !(tok->isAssignmentOp() || ((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")) || ((tok->tokType() == Token::eBracket) && tok->str() == MatchCompiler::makeConstString("{"))))
        return false;
    return true;
}
// pattern: new
...
// pattern: alignof|_Alignof|_alignof|__alignof|__alignof__|decltype|offsetof|sizeof|typeid|typeof|__typeof__ (
MAYBE_UNUSED static inline bool match149(const Token* tok) {
    if (!tok || !((tok->str() == MatchCompiler::makeConstString("alignof")) || (tok->str() == MatchCompiler::makeConstString("_Alignof")) || (tok->str() == MatchCompiler::makeConstString("_alignof")) || (tok->str() == MatchCompiler::makeConstString("__alignof")) || (tok->str() == MatchCompiler::makeConstString("__alignof__")) || (tok->str() == MatchCompiler::makeConstString("decltype")) || (tok->str() == MatchCompiler::makeConstString("offsetof")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("sizeof")) || (tok->str() == MatchCompiler::makeConstString("typeid")) || (tok->str() == MatchCompiler::makeConstString("typeof")) || (tok->str() == MatchCompiler::makeConstString("__typeof__"))))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")))
        return false;
...
#line 1 "/opt/cppcheck/cppcheck-main/lib/astutils.cpp"
/*
...
{
    return match4(tok->astParent()) && match5(tok->astParent()->astParent());
}
...
std::string astCanonicalType(const Token *expr, bool pointedToType)
{
    ...
    if (decl.first && decl.second) {
        std::string ret;
        for (const Token *type = decl.first; match6(type) && type != decl.second; type = type->next()) {
            if (!match7(type))
                ret += type->str();
        }
        return ret;
...
const Token * astIsVariableComparison(const Token *tok, const std::string &comp, const std::string &rhs, const Token **vartok)
{
    ...
    } else if (comp == MatchCompiler::makeConstString("!=") && rhs == MatchCompiler::makeConstString("0")) {
        if (tok->str() == MatchCompiler::makeConstString("!")) {
            ret = tok->astOperand1();
            // handle (!(x==0)) as (x!=0)
    ...
    while (ret && ret->str() == MatchCompiler::makeConstString("."))
        ret = ret->astOperand2();
    if (ret && ret->str() == MatchCompiler::makeConstString("=") && ret->astOperand1() && ret->astOperand1()->varId())
        ret = ret->astOperand1();
...
bool isVariableDecl(const Token* tok)
{
    ...
    return match8(varDeclEndToken) && varDeclEndToken->next() == tok;
}

bool isStlStringType(const Token* tok)
{
    return match9(tok) ||
           (match10(tok) && !match11(tok->linkAt(3)));
}

bool isVoidCast(const Token* tok)
{
    return match5(tok) && tok->isCast() && tok->valueType() &&
...
bool isTemporary(const Token* tok, const Library* library, bool unknown)
{
    if (!tok)
        return false;
    if (match12(tok))
        return (tok->originalName() != MatchCompiler::makeConstString("->") && isTemporary(tok->astOperand1(), library)) ||
    ...
    if (tok->isUnaryOp("*"))
        return false;
    if (match15(tok) && isLikelyStream(tok->astOperand1()))
        return false;
    if (match16(tok)) {
    ...
    if (match5(tok) && tok->astOperand1() &&
        (tok->astOperand2() || match17(tok->next()))) {
        if (match18(tok->astOperand1()))
            return false;
        if (tok->valueType()) {
            if (tok->valueType()->pointer > 0) {
                ...
                if (match19(parent))
                    return true;
                if (match20(parent) && parent->valueType()->reference != Reference::None &&
        ...
        if (match21(tok->previous()) && tok->linkAt(-1))
    ...
    if (match5(tok) && match5(tok->astOperand1()))
        return unknown;
    if (match22(tok) && match20(tok->astParent()) && tok->astOperand1() &&
        !tok->astOperand2())
...
static bool isFunctionCall(const Token* tok)
{
    if (match23(tok))
        return true;
    if (match24(tok) && match25(tok->linkAt(1)))
        return true;
    if (match26(tok))
...
static T* nextAfterAstRightmostLeafGeneric(T* tok)
{
    ...
    while (match27(rightmostLeaf->next()) && !hasToken(rightmostLeaf->linkAt(1), rightmostLeaf->next(), tok))
        rightmostLeaf = rightmostLeaf->next();
    if (match28(rightmostLeaf) && rightmostLeaf->link())
...
Token* astParentSkipParens(Token* tok)
{
    ...
    if (parent->link() != nextAfterAstRightmostLeaf(tok))
        return parent;
    if (match23(parent->previous()) ||
        (match25(parent->previous()) && parent->linkAt(-1)))
        return parent;
...
static std::vector<const Token*> getParentMembers(const Token* tok)
{
    if (!tok)
        return {};
    if (!match12(tok->astParent()))
        return {tok};
    const Token* parent = tok->astParent();
    while (match12(parent->astParent()))
        parent = parent->astParent();
    std::vector<const Token*> result;
    for (const Token* tok2 : astFlatten(parent, ".")) {
        if (match5(tok2) && match12(tok2->astOperand1())) {
    ...
    if (std::any_of(it.base() - 1, members.cend() - 1, [&](const Token* tok2) {
        ...
        if (!astIsUniqueSmartPointer(obj)) {
            ...
            if (!match12(dotTok)) {
                ...
                if (!endTok)
                    dotTok = obj->next();
                else if (match12(endTok))
                    dotTok = endTok;
                else if (match12(endTok->next()))
                    dotTok = endTok->next();
            }
            // If we are dereferencing the member variable then treat it as borrowed
            if (match12(dotTok) && dotTok->originalName() == MatchCompiler::makeConstString("->"))
                return true;
        }
...
static bool isInConstructorList(const Token* tok)
{
    ...
    do {
        parent = parent->astParent();
    } while (match33(parent));
    return match4(parent) && !match16(parent->astParent());
}
...
std::vector<ValueType> getParentValueTypes(const Token* tok, const Settings& settings, const Token** parent)
{
    ...
    if (match35(tok->astParent()->tokAt(-2)) &&
    ...
    if (match5(tok->astParent()) && ftok && !tok->astParent()->isCast() &&
        ftok->tokType() != Token::eType)
        return {};
    if (parent && match36(tok->astParent())) {
...
static T* getCondTokFromEndImpl(T* endBlock)
{
    ...
    if (!match22(startBlock))
        return nullptr;
    if (match42(startBlock->previous()))
        return getCondTok(startBlock->previous());
    if (match17(startBlock->previous()))
        return getCondTok(startBlock->linkAt(-1));
    if (match43(startBlock->tokAt(-2)))
...
static T* getInitTokImpl(T* tok)
{
    ...
    if (tok->str() != MatchCompiler::makeConstString("("))
        return nullptr;
    if (!match40(tok->astOperand2()))
        return nullptr;
    if (match40(tok->astOperand2()->astOperand1()))
        return nullptr;
...
static T* getStepTokImpl(T* tok)
{
    ...
    if (!match40(tok->astOperand2()))
        return nullptr;
    if (!match40(tok->astOperand2()->astOperand2()))
        return nullptr;
...
const Token *findNextTokenFromBreak(const Token *breakToken)
{
    ...
    while (scope) {
        if (scope->isLoopScope() || scope->type == ScopeType::eSwitch) {
            if (scope->type == ScopeType::eDo && match38(scope->bodyEnd))
...
{
    if (!match44(forToken) || !match40(forToken->next()->astOperand2()))
        return false;
    ...
    if (!initExpr || !initExpr->isBinaryOp() || initExpr->str() != MatchCompiler::makeConstString("=") || !match45(initExpr->astOperand1()))
        return false;
    ...
    partialCond = match46(condExpr);
    visitAstNodes(condExpr, [varid, &condExpr](const Token *tok) {
        if (match46(tok))
            return ChildrenToVisit::op1_and_op2;
        if (match47(tok) && tok->isBinaryOp() && tok->astOperand1()->varId() == varid && tok->astOperand2()->hasKnownIntValue()) {
            if (match46(condExpr) || tok->astOperand2()->getKnownIntValue() < condExpr->astOperand2()->getKnownIntValue())
                condExpr = tok;
        }
        return ChildrenToVisit::none;
    });
    if (!match47(condExpr) || !condExpr->isBinaryOp() || condExpr->astOperand1()->varId() != varid || !condExpr->astOperand2()->hasKnownIntValue())
        return false;
    if (!incExpr || !incExpr->isUnaryOp("++") || incExpr->astOperand1()->varId() != varid)
        return false;
    stepValue = 1;
    if (condExpr->str() == MatchCompiler::makeConstString("<"))
...
static const Token * getVariableInitExpression(const Variable * var)
{
    ...
    if (!varDeclEndToken)
        return nullptr;
    if (match48(varDeclEndToken, var->declarationId()))
...
const Token* isInLoopCondition(const Token* tok)
{
    ...
    return match49(top->previous()) ? top : nullptr;
}
...
bool exprDependsOnThis(const Token* expr, bool onVar, nonneg int depth)
{
    if (!expr)
        return false;
    if (expr->str() == MatchCompiler::makeConstString("this"))
        return true;
    if (depth >= 1000)
    ...
    if (match23(expr)) {
        ...
        if (expr->isOperatorKeyword() && !match12(expr->next()->astParent()))
            return true;
    }
...
bool isStructuredBindingVariable(const Variable* var)
{
    ...
    while (tok && match51(tok->astParent()))
        tok = tok->astParent();
    return tok && (tok->str() == MatchCompiler::makeConstString("[") || match52(tok->previous())); // TODO: remove workaround when #11105 is fixed
}
                                                ...
                                                int depth)
{
    ...
    } else if (match16(tok) && match4(tok->astOperand2())) {
    ...
    } else if (tok->previous() && tok->previous()->function() && match23(tok->previous())) {
...
static inline bool isSameConstantValue(bool macro, const Token* tok1, const Token* tok2)
{
    ...
    auto adjustForCast = [](const Token* tok) {
        if (tok->astOperand2() && match56(tok->previous()) && tok->previous()->isStandardType())
...
static bool isForLoopCondition(const Token * const tok)
{
    if (!tok)
        return false;
    const Token *const parent = tok->astParent();
    return match40(parent) && parent->astOperand1() == tok &&
           match40(parent->astParent()) &&
           match5(parent->astParent()->astParent()) &&
           parent->astParent()->astParent()->astOperand1()->str() == MatchCompiler::makeConstString("for");
}
...
static bool isForLoopIncrement(const Token* const tok)
{
    if (!tok)
        return false;
    const Token *const parent = tok->astParent();
    return match40(parent) && parent->astOperand2() == tok &&
           match40(parent->astParent()) &&
           match5(parent->astParent()->astParent()) &&
           parent->astParent()->astParent()->astOperand1()->str() == MatchCompiler::makeConstString("for");
}
...
bool isUsedAsBool(const Token* const tok, const Settings& settings)
{
    ...
    if (match58(parent))
        return true;
    if (parent->isCast())
        return !match59(parent->astOperand1()) && isUsedAsBool(parent, settings);
    if (parent->isUnaryOp("*"))
        return isUsedAsBool(parent, settings);
    if (match60(parent) && (tok->astSibling()->isNumber() || tok->astSibling()->isKeyword()) && tok->astSibling()->hasKnownIntValue() &&
        tok->astSibling()->getKnownIntValue() == 0)
        return true;
    if (parent->str() == MatchCompiler::makeConstString("(") && astIsRHS(tok) && match61(parent->astOperand1()))
        return true;
    if (match16(parent) && astIsLHS(tok))
        return true;
    if (isForLoopCondition(tok))
        return true;
    if (!match62(parent) && !(parent->str() == MatchCompiler::makeConstString("(") && tok == parent->astOperand1())) {
        if (parent->str() == MatchCompiler::makeConstString(",") && parent->isInitComma())
            return false;
...
bool isSameExpression(bool macro, const Token *tok1, const Token *tok2, const Settings& settings, bool pure, bool followVar, ErrorPath* errors)
{
    ...
    // Skip double not
    if (match63(tok1) && match63(tok1->astOperand1()) && !match55(tok1->astParent()) && astIsBoolLike(tok2, settings)) {
        return isSameExpression(macro, tok1->astOperand1()->astOperand1(), tok2, settings, pure, followVar, errors);
    }
    if (match63(tok2) && match63(tok2->astOperand1()) && !match55(tok2->astParent()) && astIsBoolLike(tok1, settings)) {
    ...
    while (match64(followTok1) && followTok1->astOperand2())
        followTok1 = followTok1->astOperand2();
    while (match64(followTok2) && followTok2->astOperand2())
    ...
    if (tok1->varId() != tok2->varId() || !tok_str_eq || tok1->originalName() != tok2->originalName()) {
        if ((match65(tok1) && match65(tok2)) ||
            (match66(tok1) && match66(tok2))) {
        ...
        if (match60(tok1)) {
            condTok = tok1;
            exprTok = tok2;
        } else if (match60(tok2)) {
            condTok = tok2;
            exprTok = tok1;
        }
        if (condTok && condTok->astOperand1() && condTok->astOperand2() && !match67(exprTok)) {
            ...
            const bool exprIsNot = match63(exprTok);
            if (exprIsNot)
                varTok2 = exprTok->astOperand1();
            bool compare = false;
            if (value) {
                if (value->intvalue == 0 && exprIsNot && match68(condTok)) {
                    compare = true;
                } else if (value->intvalue == 0 && !exprIsNot && match69(condTok)) {
                    compare = true;
                } else if (value->intvalue != 0 && exprIsNot && match69(condTok)) {
                    compare = true;
                } else if (value->intvalue != 0 && !exprIsNot && match68(condTok)) {
                    compare = true;
                }
            }
    ...
    if (pure && tok1->isName() && tok1->strAt(1) == MatchCompiler::makeConstString("(") && tok1->str() != MatchCompiler::makeConstString("sizeof") && !(tok1->variable() && tok1 == tok1->variable()->nameToken())) {
        if (!tok1->function()) {
            if (match12(tok1->previous())) {
                ...
                const bool lhsIsConst = (lhs->variable() && lhs->variable()->isConst()) ||
                                        ...
                                        (match71(lhs) && settings.library.isFunctionConst(lhs->tokAt(2)));
                if (!lhsIsConst)
                    return false;
            } else {
    ...
    // templates/casts
    if ((tok1->next() && tok1->linkAt(1) && match24(tok1)) ||
        (tok2->next() && tok2->linkAt(1) && match24(tok2))) {

        // non-const template function that is not a dynamic_cast => return false
        if (pure && match25(tok1->linkAt(1)) &&
    ...
    if (tok1->str() == MatchCompiler::makeConstString("(") && tok1->previous() &&
        ...
        !(tok1->strAt(-1) == MatchCompiler::makeConstString(">") && tok1->linkAt(-1))) {
        const Token *t1 = tok1->next();
        const Token *t2 = tok2->next();
        while (t1 && t2 &&
               t1->str() == t2->str() &&
    ...
    if (tok1->isCpp() && tok1->str() == MatchCompiler::makeConstString("+") && tok1->isBinaryOp()) {
    ...
    const bool commutative = tok1->isBinaryOp() && match72(tok1);
...
bool isOppositeCond(bool isNot, const Token * const cond1, const Token * const cond2, const Settings& settings, bool pure, bool followVar, ErrorPath* errors)
{
    ...
    if (!isNot && cond1->str() == MatchCompiler::makeConstString("&&") && cond2->str() == MatchCompiler::makeConstString("&&")) {
    ...
    if (cond1->str() != cond2->str() && (cond1->str() == MatchCompiler::makeConstString("||") || cond2->str() == MatchCompiler::makeConstString("||"))) {
...
bool isConstFunctionCall(const Token* ftok, const Library& library)
{
    ...
    if (const Function* f = ftok->function()) {
        ...
        if (match12(ftok->previous()) || exprDependsOnThis(ftok->next())) {
            if (f->isConst())
                return true;
    ...
    } else if (match73(ftok->previous()) && ftok->previous()->originalName() != MatchCompiler::makeConstString("->") &&
               astIsSmartPointer(ftok->previous()->astOperand1())) {
        return match74(ftok);
    } else if (match73(ftok->previous()) && astIsContainer(ftok->previous()->astOperand1())) {
        ...
        const bool memberFunction = match73(ftok->previous());
        bool constMember = !memberFunction;
        if (match71(ftok->tokAt(-2))) {
...
bool isConstExpression(const Token *tok, const Library& library)
{
    ...
    if (tok->variable() && tok->variable()->isVolatile())
        return false;
    if (tok->isName() && tok->strAt(1) == MatchCompiler::makeConstString("(")) {
...
static bool hasNoreturnFunction(const Token* tok, const Library& library, const Token** unknownFunc)
{
    if (!tok)
        return false;
    const Token* ftok = tok->str() == MatchCompiler::makeConstString("(") ? tok->previous() : nullptr;
    while (match5(ftok))
...
bool isReturnScope(const Token* const endToken, const Library& library, const Token** unknownFunc, bool functionScope)
{
    ...
    while (prev && match79(prev->previous()))
        prev = prev->previous();
    if (prev && match80(prev->previous()))
        prev = prev->previous();

    if (match41(prev)) {
        if (match43(prev->link()->tokAt(-2)))
        ...
        if (!functionScope && match81(prev->link()->previous()) &&
            match82(prev->link()->linkAt(-1)->previous()) &&
            !findmatch83(prev->link(), prev) ) {
            return isReturnScope(prev, library, unknownFunc, functionScope);
        }
        if (isEscaped(prev->link()->astTop(), functionScope, library))
            return true;
        if (match84(prev->link()->previous()))
            return isReturnScope(prev, library, unknownFunc, functionScope);
    } else if (match40(prev)) {
        if (prev->tokAt(-2) && hasNoreturnFunction(prev->tokAt(-2)->astTop(), library, unknownFunc))
            return true;
        // Unknown symbol
        if (match85(prev->tokAt(-2)) && prev->previous()->isIncompleteVar()) {
            if (unknownFunc)
                *unknownFunc = prev->previous();
            return false;
        }
        if (match86(prev->previous()) && prev->linkAt(-1) &&
        ...
        prev = prev->previous();
        while (prev && !match87(prev) && !isEscapedOrJump(prev, functionScope, library))
            prev = prev->previous();
...
static T* getTokenArgumentFunctionImpl(T* tok, int& argn)
{
    argn = -1;
    {
        ...
        while (parent && parent->isCast())
            parent = parent->astParent();
        if (match89(parent) && parent->valueType() && parent->valueType()->pointer)
            parent = parent->astParent();

        // passing variable to subfunction?
        if (match90(parent) || match46(parent) || (parent && parent->isUnaryOp("*")))
            ;
        else if (match4(parent)) {
            while (match91(parent))
                parent = parent->astParent();
            while (match33(parent))
                parent = parent->astParent();
    ...
    T* argtok = tok;
    while (argtok && argtok->astParent() && (!match92(argtok->astParent()) || argtok->astParent()->isCast())) {
    ...
    if (tok->astOperand2())
        tok = tok->astOperand1();
    while (tok && (tok->isUnaryOp("*") || tok->str() == MatchCompiler::makeConstString("[")))
        tok = tok->astOperand1();
    if (match94(tok)) // bailout for pointer to member
        return tok->tokAt(2);
    while (match12(tok))
        tok = tok->astOperand2();
    while (match64(tok)) {
...
std::vector<const Variable*> getArgumentVars(const Token* tok, int argnr)
{
    ...
    if (tok->variable() || match22(tok) || match56(tok->previous())) {
        ...
        if (!typeScope)
            return {};
        const bool tokIsBrace = match22(tok);
        ...
        for (const Function &function : typeScope->functionList) {
            ...
            if (constructor && !function.isConstructor())
                continue;
            if (!constructor && !match98(function.token))
                continue;
...
bool isMutableExpression(const Token* tok)
{
    ...
    if (match100(tok))
        return false;
    if (tok->previous() && tok->previous()->isKeyword() && match23(tok->previous()))
        return false;
    if (tok->link() && match65(tok))
        return false;
    if (tok->astOperand1() && match30(tok))
...
bool isVariableChangedByFunctionCall(const Token *tok, int indirect, const Settings &settings, bool *inconclusive)
{
    ...
    if (indirect == 1 && tok->isCpp() && tok->tokAt(-1) && match101(tok->tokAt(-2))) // placement new TODO: fix AST
        return true;
    ...
    if (match22(tok) && isTrivialConstructor(tok))
        return false;
    if (tok->isKeyword() && !isCPPCastKeyword(tok) && !startsWith(tok->str(),"operator"))
        return false;
    // A functional cast won't modify the variable
    if (match56(tok) && tok->tokType() == Token::eType && astIsPrimitive(tok->next()))
        return false;
    const Token * parenTok = tok->next();
    if (match95(parenTok) && parenTok->link())
        parenTok = parenTok->link()->next();
    const bool possiblyPassedByReference = (parenTok->next() == tok1 || match102(tok1->previous()));
    ...
    if (addressOf && tok1->astParent()->isUnaryOp("&")) {
        ...
        if (match104(castToken->astParent()) &&
            castToken->valueType() &&
...
bool isVariableChanged(const Token *tok, int indirect, const Settings &settings, int depth)
{
    ...
    while ((tok2->astParent() && tok2->astParent()->isUnaryOp("*")) ||
           (match12(tok2->astParent()) && !match105(tok2->astParent()->astParent())) ||
           (tok2->astParent() && tok2->astParent()->isUnaryOp("&") && match12(tok2->astParent()->astParent()) && tok2->astParent()->astParent()->originalName()==MatchCompiler::makeConstString("->")) ||
           (match30(tok2->astParent()) && tok2 == tok2->astParent()->astOperand1())) {
        if (tok2->astParent() && (tok2->astParent()->isUnaryOp("*") || (astIsLHS(tok2) && tok2->astParent()->originalName() == MatchCompiler::makeConstString("->"))))
            derefs++;
        if (derefs > indirect)
            break;
        if (tok2->astParent() && tok2->astParent()->isUnaryOp("&") && match12(tok2->astParent()->astParent()) && tok2->astParent()->astParent()->originalName()==MatchCompiler::makeConstString("->"))
            tok2 = tok2->astParent();
        tok2 = tok2->astParent();
    }
    ...
    while ((match4(tok2) && match16(tok2->astParent())) ||
           (match4(tok2->astParent()) && match16(tok2->astParent()->astParent())))
        tok2 = tok2->astParent();
    ...
    if (tok2->isCpp() && match106(tok2->astParent()) && astIsRHS(tok2) && isLikelyStreamRead(tok2->astParent()))
        return true;
    ...
    // Member function call
    if (match107(tok2->astParent()) && isFunctionCall(tok2->astParent()->next()) &&
        ...
        const Token* const ctok = tok2->str() == MatchCompiler::makeConstString(".") ? tok2->astOperand2() : tok2;
        ...
        if (settings.library.isFunctionConst(ftok) || (astIsSmartPointer(tok) && ftok->str() == MatchCompiler::makeConstString("get"))) // TODO: replace with action/yield?
            return false;
    ...
    // Member pointer
    if (match108(tok2->astParent())) {
        const Token* ftok = tok2->astParent()->linkAt(2)->previous();
        // TODO: Check for pointer to member variable
        if (!ftok->function() || !ftok->function()->isConst())
            return true;
    }
    if (match94(tok2->astParent())) // bailout
        return true;

    if (match30(tok2) && astIsContainer(tok) && vt && vt->container && vt->container->stdAssociativeLike)
        return true;

    const Token *ftok = tok2;
    while (ftok && (!match109(ftok) || ftok->isCast()))
        ftok = ftok->astParent();

    if (ftok && match110(ftok->link())) {
        if (ftok->str() == MatchCompiler::makeConstString("(") && match30(ftok->astOperand1())) // operator() on array element, bail out
            return true;
        const Token * ptok = tok2;
        while (match111(ptok->astParent()))
            ptok = ptok->astParent();
        int pindirect = indirect;
        if (indirect == 0 && astIsLHS(tok2) && match112(ptok) && astIsPointer(ptok->next()))
            pindirect = 1;
    ...
    if (match4(tok2->astParent()) && tok2->astParent()->astParent() && match44(tok2->astParent()->astParent()->previous())) {
...
const Token* findEscapeStatement(const Scope* scope, const Library& library)
{
    ...
    for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
        ...
        if (!tok->isKeyword())
            continue;
        if (match118(tok)) // TODO: check try/catch, labels?
            return tok;
        if (!match119(tok))
            continue;
...
bool isVariableChanged(const Variable * var, const Settings &settings, int depth)
{
    ...
    if (!start)
        return false;
    if (match48(start, var->declarationId()))
        start = start->tokAt(2);
    if (match55(start)) {
...
{
    if ((match23(tok->previous()) && !match12(tok->astOperand1())) ||
        match121(tok->tokAt(-3))) {
...
int numberOfArgumentsWithoutAst(const Token* start)
{
    ...
    while (match17(openBracket))
        openBracket = openBracket->next();
    if (openBracket && openBracket->str()==MatchCompiler::makeConstString("(") && openBracket->next() && openBracket->strAt(1)!=MatchCompiler::makeConstString(")")) {
...
const Token* getIteratorExpression(const Token* tok)
{
    ...
    if (!tok->isName()) {
        ...
        if (iter2)
            return iter2;
    } else if (match123(tok)) {
        if (match124(tok->previous()))
            return tok->previous()->astOperand1();
        if (!match12(tok->previous()) && match125(tok) &&
            !match126(tok->linkAt(1)))
            return tok->next()->astOperand2();
    }
    return nullptr;
}
...
{
    if (!last || !last->isCpp() || last->str() != MatchCompiler::makeConstString("}"))
        return nullptr;
    const Token* tok = last->link();
    if (match5(tok->astParent()))
        tok = tok->astParent();
    if (match30(tok->astParent()))
...
static T* findLambdaEndTokenGeneric(T* first)
{
    ...
    if (!first || !first->isCpp() || first->str() != MatchCompiler::makeConstString("["))
        return nullptr;
    if (!maybeLambda(first->previous()))
        return nullptr;
    if (!match128(first->link()))
        return nullptr;
    const Token* roundOrCurly = first->link()->next();
    if (roundOrCurly->link() && roundOrCurly->str() == MatchCompiler::makeConstString("<"))
...
bool isLikelyStream(const Token *stream)
{
    ...
    if (!match15(stream->astParent()) || !stream->astParent()->isBinaryOp())
        return false;
...
bool isLikelyStreamRead(const Token *op)
{
    ...
    if (!match129(op) || !op->isBinaryOp())
        return false;

    if (!match130(op->astOperand2()) && op->str() != op->astOperand2()->str())
        return false;

    const Token *parent = op;
    while (parent->astParent() && parent->astParent()->str() == op->str())
        parent = parent->astParent();
    if (parent->astParent() && !match131(parent->astParent()))
        return false;
...
{
    return tok && match25(tok->previous()) && tok->astOperand2() && tok->astOperand1() && isCPPCastKeyword(tok->astOperand1());
}
...
bool isConstVarExpression(const Token *tok, const std::function<bool(const Token*)>& skipPredicate)
{
    if (!tok)
        return false;
    if (tok->str() == MatchCompiler::makeConstString("?") && tok->astOperand2() && tok->astOperand2()->str() == MatchCompiler::makeConstString(":")) // ternary operator
    ...
    if (skipPredicate && skipPredicate(tok))
        return false;
    if (match132(tok->previous()))
        return true;
    if (match23(tok->previous())) {
        if (match12(tok->astOperand1()) && !isConstVarExpression(tok->astOperand1(), skipPredicate))
            return false;
    ...
    if (tok->str() == MatchCompiler::makeConstString("::") && tok->hasKnownValue())
        return isConstVarExpression(tok->astOperand2(), skipPredicate);
    if (match134(tok)) {
...
bool isLeafDot(const Token* tok)
{
    ...
    if (!match12(parent))
        return false;
    if (parent->astOperand2() == tok && !match12(parent->astParent()))
        return true;
...
ExprUsage getExprUsage(const Token* tok, int indirect, const Settings& settings)
{
    ...
    if (indirect > 0 && parent) {
        while (match30(parent) && parent->astParent())
            parent = parent->astParent();
        if (match54(parent) && (astIsRHS(tok) || astIsLHS(parent->astOperand1())))
            return ExprUsage::NotUsed;
        if (match137(parent))
            return ExprUsage::NotUsed;
        if (parent->isConstOp())
        ...
        if (parent->isCast())
            return ExprUsage::NotUsed;
        if (match4(parent) && match16(parent->astParent()))
    ...
    if (tok->isUnaryOp("&") && !parent)
        return ExprUsage::NotUsed;
    if (indirect == 0) {
        if (match138(parent) && parent->str() != MatchCompiler::makeConstString("=") &&
            !parent->isUnaryOp("&") &&
        ...
        if (match55(parent) && astIsRHS(tok)) {
        ...
        if (((match5(parent) && !parent->isCast()) || (match30(parent) && tok->valueType())) &&
            (astIsLHS(tok) || match140(parent)))
            return ExprUsage::Used;
    }
...
static const Token* getLHSVariableRecursive(const Token* tok)
{
    ...
    if (match141(tok)) {
        ...
        if ((vartok && vartok->variable()) || match30(tok))
            return vartok;
        return getLHSVariableRecursive(tok->astOperand2());
    }
    if (match142(tok->previous()))
        return tok->next();
    return tok;
}
...
const Token* findAllocFuncCallToken(const Token *expr, const Library &library)
{
    ...
    if (match23(expr->previous()) && library.getAllocFuncInfo(expr->astOperand1()))
        return expr->astOperand1();
    return (match144(expr) && expr->astOperand1()) ? expr : nullptr;
}
...
bool isNullOperand(const Token *expr)
{
    if (!expr)
        return false;
    if (expr->isCpp() && match145(expr))
        expr = expr->astParent();
    else if (!expr->isCast())
        return match146(expr);
    if (expr->valueType() && expr->valueType()->pointer == 0)
        return false;
    const Token *castOp = expr->astOperand2() ? expr->astOperand2() : expr->astOperand1();
    return match146(castOp) || (MathLib::isInt(castOp->str()) && MathLib::isNullValue(castOp->str()));
}

bool isGlobalData(const Token *expr)
{
    // function call that returns reference => assume global data
    if (expr && expr->str() == MatchCompiler::makeConstString("(") && expr->valueType() && expr->valueType()->reference != Reference::None) {
        if (expr->isBinaryOp())
            return true;
    ...
    visitAstNodes(expr,
        ...
        if (tok->originalName() == MatchCompiler::makeConstString("->")) {
            // TODO check if pointer points at local data
            globalData = true;
            return ChildrenToVisit::none;
        }
        if (match147(tok) && tok->astOperand1() && tok->astOperand1()->variable()) {
            // TODO check if pointer points at local data
        ...
        if (tok->variable()) {
            ...
            if (tok->strAt(-1) != MatchCompiler::makeConstString(".") && !tok->variable()->isLocal() && !tok->variable()->isArgument()) {
                globalData = true;
                return ChildrenToVisit::none;
            }
        ...
        if (tok->isCpp() && tok->str() == MatchCompiler::makeConstString(".") && tok->astOperand1() && tok->astOperand1()->variable() && !tok->astOperand1()->valueType()) {
            globalData = true;
            return ChildrenToVisit::none;
        }
        if (match148(tok))
...
static std::set<MathLib::bigint> getSwitchValues(const Token *startbrace, bool &hasDefault)
{
    ...
    for (const Token *tok = startbrace->next(); tok && tok != endbrace; tok = tok->next()) {
        if (match22(tok) && tok->scope()->type == ScopeType::eSwitch) {
            tok = tok->link();
            continue;
        }
        if (match150(tok)) {
...
{
    if (!startbrace || !match81(startbrace->previous()) || startbrace->scope()->type != ScopeType::eSwitch)
        return false;
...
bool isUnreachableOperand(const Token *tok)
{
    for (;;)
    {
        ...
        if (parent->isBinaryOp()) {
            ...
            // logical and
            if (match152(parent) && !left && sibling->hasKnownIntValue()
                && !sibling->getKnownIntValue())
                return true;

            // logical or
            if (match153(parent) && !left && sibling->hasKnownIntValue()
                && sibling->getKnownIntValue())
                return true;
...
static const Token *skipUnreachableIfBranch(const Token *tok)
{
    ...
    if (!match154(condTok->tokAt(-1)) && !match155(condTok->tokAt(-2)))
        return tok;
...
static const Token *skipUnreachableElseBranch(const Token *tok)
{
    ...
    if (!match156(condTok->tokAt(-1)) && !match157(condTok->tokAt(-2)))
        return tok;
