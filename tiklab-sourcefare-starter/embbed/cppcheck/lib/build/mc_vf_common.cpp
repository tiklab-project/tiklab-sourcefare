...
#line 1 "/opt/cppcheck/cppcheck-main/lib/vf_common.cpp"
/*
    ...
    Token * valueFlowSetConstantValue(Token *tok, const Settings &settings)
    {
        if ((tok->isNumber() && MathLib::isInt(tok->str())) || (tok->tokType() == Token::eChar)) {
            ...
            if (!tok->isTemplateArg())
                value.setKnown();
            setTokenValue(tok, std::move(value), settings);
        } else if (tok->str() == MatchCompiler::makeConstString("NULL") || ((tok->isCpp() || settings.standards.c >= Standards::C23) && tok->str() == MatchCompiler::makeConstString("nullptr"))) {
            Value value(0);
            if (!tok->isTemplateArg())
                value.setKnown();
            setTokenValue(tok, std::move(value), settings);
            ...
            while (tok2 && !tok2->isStandardType() && match2(tok2)) {
            ...
            if (match3(tok)) {
                ...
                setTokenValue(tok, value, settings);
                setTokenValue(tok->next(), std::move(value), settings);
            } else if (match4(tok) && tok->next()->astParent() == tok->tokAt(4) &&
                       tok->tokAt(4)->astOperand2() && match1(tok->tokAt(4)->astOperand2()->previous())) {
                ...
                if (varid1 &&
                    ...
                    match5(sz2->astOperand2()) && match6(sz2->astOperand2()->astOperand1(), varid1)) {
            ...
            // skip over enum
            tok = tok->linkAt(1);
        } else if (match9(tok) && (tok->isStandardType() ||
