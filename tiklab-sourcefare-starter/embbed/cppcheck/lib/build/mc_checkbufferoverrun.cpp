...
MAYBE_UNUSED static inline bool match13(const Token* tok) {
    if (!tok || !(tok->str() == MatchCompiler::makeConstString("strncpy")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")))
        return false;
...
#line 1 "/opt/cppcheck/cppcheck-main/lib/checkbufferoverrun.cpp"
/*
    ...
    if (array->variable() && array->variable()->isArray() && !array->variable()->dimensions().empty()) {
        ...
        if (dimensions[0].num <= 1 || !dimensions[0].tok) {
            visitAstNodes(arrayToken,
                          [&](const Token *child) {
                if (child->originalName() == MatchCompiler::makeConstString("->")) {
                    mightBeLarger = true;
                                                           ...
                                                           MathLib::bigint path)
{
    ...
    bool isArrayIndex = tok->str() == MatchCompiler::makeConstString("[");
    if (isArrayIndex) {
...
void CheckBufferOverrun::arrayIndex()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (!array->scope()->isExecutable()) {
            ...
            const Token *parent = tok;
            while (parent && !match3(parent->astParent()))
                parent = parent->astParent();
        ...
        for (const Token *tok2 = tok; tok2 && tok2->str() == MatchCompiler::makeConstString("["); tok2 = tok2->link()->next()) {
            if (!tok2->astOperand2()) {
                indexTokens.clear();
                break;
            }
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token *tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            ...
            for (int argnr = 0; argnr < args.size(); ++argnr) {
                ...
                if (bufferSize.intvalue == 1 && match7(argtok->astParent())) { // TODO: check if parent was allocated dynamically
                    const Token *tok2 = argtok;
                    while (match7(tok2->astParent()))
                        tok2 = tok2->astParent();
                    while (match8(tok2))
    ...
    for (const Scope * const scope : symbolDatabase->functionScopes) {
        for (const Token *tok = scope->bodyStart; tok && tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (match10(tok)) {
                ...
                while (tok2->astParent() && tok2->tokType() != Token::eLogicalOp && tok2->str() != MatchCompiler::makeConstString("?")) {
                    tok3 = tok2;
                    tok2 = tok2->astParent();
                }
                ...
                if (match11(tok2, indexID))
                    arrayIndexThenCheckError(tok, indexName);
                else if (match12(tok2, indexID))
                    arrayIndexThenCheckError(tok, indexName);
            }
        }
    }
    ...
    for (const Scope * const scope : symbolDatabase->functionScopes) {
        for (const Token *tok = scope->bodyStart; tok && tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (bufferSize.intvalue < 0 || sizeToken->getKnownIntValue() < bufferSize.intvalue)
                continue;
            if (match14(args[1]) && match15(args[1]->astOperand1()) && args[1]->astOperand1()->astOperand1()) {
...
bool CheckBufferOverrun::isCtuUnsafeBufferUsage(const Settings &settings, const Token *argtok, CTU::FileInfo::Value *offset, int type)
{
    ...
    if (type == 1 && match18(argtok) && argtok->astParent() == argtok->next() && !match19(argtok->linkAt(1)))
        indexTok = argtok->next()->astOperand2();
    else if (type == 2 && match20(argtok->astParent()))
...
void CheckBufferOverrun::negativeArraySize()
{
    ...
    for (const Variable* var : symbolDatabase->variableList()) {
        ...
        if (!match21(nameToken) || !nameToken->next()->astOperand2())
            continue;
    ...
    for (const Scope* functionScope : symbolDatabase->functionScopes) {
        for (const Token* tok = functionScope->bodyStart; tok != functionScope->bodyEnd; tok = tok->next()) {
            if (!tok->isKeyword() || tok->str() != MatchCompiler::makeConstString("new") || !tok->astOperand1() || tok->astOperand1()->str() != MatchCompiler::makeConstString("["))
                continue;
