...
// pattern: ;
template<class T> MAYBE_UNUSED static inline T * findmatch5(T * start_tok) {
...
// pattern: %varid% =
MAYBE_UNUSED static inline bool match10(const Token* tok, const int varid) {
    if (varid==0U)
        throw InternalError(tok, "Internal error. Token::Match called with varid 0. Please report this to Cppcheck developers");
    if (!tok || !(tok->isName() && tok->varId() == varid))
        return false;
...
// pattern: %varid% ++|--
MAYBE_UNUSED static inline bool match13(const Token* tok, const int varid) {
    if (varid==0U)
        throw InternalError(tok, "Internal error. Token::Match called with varid 0. Please report this to Cppcheck developers");
    if (!tok || !(tok->isName() && tok->varId() == varid))
        return false;
...
// pattern: [(,] &| %varid% [,)]
MAYBE_UNUSED static inline bool match14(const Token* tok, const int varid) {
    if (!tok || tok->str().size() != 1U || !strchr("(,", tok->str()[0]))
        return false;
    tok = tok->next();
    if (tok && (((tok->tokType() == Token::eBitOp) && tok->str() == MatchCompiler::makeConstString("&"))))
...
MAYBE_UNUSED static inline bool match15(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("break")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("continue")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("return"))))
        return false;
    return true;
}
// pattern: %name% (
...
MAYBE_UNUSED static inline bool match31(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eBitOp) && tok->str() == MatchCompiler::makeConstString("&")) || ((tok->tokType() == Token::eComparisonOp) && tok->str() == MatchCompiler::makeConstString("=="))))
        return false;
    return true;
}
// pattern: } if (
...
MAYBE_UNUSED static inline bool match35(const Token* tok) {
    if (!tok || !(tok->isConstOp() || ((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")) || ((tok->tokType() == Token::eExtendedOp || tok->tokType() == Token::eLambda) && tok->str() == MatchCompiler::makeConstString("["))))
        return false;
    return true;
}
// pattern: %name% .
...
MAYBE_UNUSED static inline bool match37(const Token* tok) {
    if (!tok || !((tok->tokType() == Token::eBracket) && tok->str() == MatchCompiler::makeConstString("{")))
        return false;
    tok = tok->next();
    if (!tok || !(((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("return")) || (tok->str() == MatchCompiler::makeConstString("throw")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("continue")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("break"))))
        return false;
    return true;
}
// pattern: if|return
MAYBE_UNUSED static inline bool match38(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("if")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("return"))))
        return false;
    return true;
}
// pattern: %cop%
...
MAYBE_UNUSED static inline bool match41(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("case")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("break")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("continue")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("return")) || (tok->str() == MatchCompiler::makeConstString("throw"))))
        return false;
    return true;
}
// pattern: [;{}] %name% :
...
MAYBE_UNUSED static inline bool match43(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("for")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("while")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("do"))))
        return false;
    return true;
}
// pattern: do {
...
MAYBE_UNUSED static inline bool match47(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("*")) || (tok->str() == MatchCompiler::makeConstString(".")) || ((tok->tokType() == Token::eExtendedOp || tok->tokType() == Token::eLambda) && tok->str() == MatchCompiler::makeConstString("["))))
        return false;
    return true;
}
// pattern: .|[
...
MAYBE_UNUSED static inline bool match58(const Token* tok) {
    if (!tok || !(tok->isConstOp() || ((tok->tokType() == Token::eLogicalOp) && tok->str() == MatchCompiler::makeConstString("&&")) || (tok->tokType() == Token::eLogicalOp && tok->str() == MatchCompiler::makeConstString("||"))))
        return false;
    return true;
}
// pattern: if
...
MAYBE_UNUSED static inline bool match65(const Token* tok) {
    if (!tok || !(tok->isName() || ((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")) || ((tok->tokType() == Token::eExtendedOp || tok->tokType() == Token::eLambda) && tok->str() == MatchCompiler::makeConstString("[")) || (tok->str() == MatchCompiler::makeConstString("::")) || (tok->str() == MatchCompiler::makeConstString("."))))
        return false;
    return true;
}
// pattern: <
...
// pattern: static_assert|assert|ASSERT|sizeof|decltype (
MAYBE_UNUSED static inline bool match67(const Token* tok) {
    if (!tok || !((tok->str() == MatchCompiler::makeConstString("static_assert")) || (tok->str() == MatchCompiler::makeConstString("assert")) || (tok->str() == MatchCompiler::makeConstString("ASSERT")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("sizeof")) || (tok->str() == MatchCompiler::makeConstString("decltype"))))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")))
        return false;
...
MAYBE_UNUSED static inline bool match76(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eExtendedOp || tok->tokType() == Token::eLambda) && tok->str() == MatchCompiler::makeConstString("[")) || ((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")) || ((tok->tokType() == Token::eBitOp) && tok->str() == MatchCompiler::makeConstString("&")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("+")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("-")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("*")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("/")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("%")) || ((tok->tokType() == Token::eBitOp) && tok->str() == MatchCompiler::makeConstString("^")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString(">>")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("<<"))))
        return false;
    return true;
}
// pattern: ( )
...
#line 1 "/opt/cppcheck/cppcheck-main/lib/checkcondition.cpp"
/*
...
bool CheckCondition::isAliased(const std::set<int> &vars) const
{
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        if (match2(tok) && vars.find(tok->tokAt(2)->varId()) != vars.end())
            return true;
    }
    return false;
}
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (match3(tok->tokAt(-2))) {
            ...
            if (match4(tok->next())) {
                ...
                const Token *endToken = findmatch5(tok) ;

                // Casting address
                if (endToken && match6(endToken->tokAt(-4)))
                    endToken = nullptr;

                if (endToken && match7(endToken->tokAt(-2))) {
                                        ...
                                        const MathLib::bigint num)
{
    ...
    for (const Token *tok2 = startTok; tok2; tok2 = tok2->next()) {
        if ((bitop == '&') && match9(tok2->tokAt(2), varid) && tok2->strAt(3) == std::string(1U, bitop)) {
            const MathLib::bigint num2 = MathLib::toBigNumber(tok2->tokAt(4));
            if (0 == (num & num2))
                mismatchingBitAndError(assignTok, num, tok2, num2);
        }
        if (match10(tok2, varid)) {
            return true;
        }
        if (bitop == '&' && match11(tok2, varid)) {
            const MathLib::bigint num2 = MathLib::toBigNumber(tok2->tokAt(2));
            if (0 == (num & num2))
                mismatchingBitAndError(assignTok, num, tok2, num2);
        }
        if (match12(tok2, varid) || match13(tok2, varid))
            return true;
        if (match14(tok2, varid) && isParameterChanged(tok2))
            return true;
        ...
        if (ret && tok2->str() == MatchCompiler::makeConstString(";"))
            return false;
        if (!islocal && match16(tok2) && !match17(tok2->linkAt(1)))
            return true;
        if (match18(tok2)) {
            ...
            if (tok2->str() == MatchCompiler::makeConstString("while")) {
                ...
                if (!bodyEnd || bodyEnd->str() != MatchCompiler::makeConstString("}") || isVariableChanged(bodyStart, bodyEnd, varid, !islocal, *mSettings))
                    continue;
            }
            ...
            for (; tok2 != end; tok2 = tok2->next()) {
                if (match14(tok2, varid)) {
                    return true;
                }
                if (match19(tok2, varid)) {
                ...
                if (match20(tok2, varid) && tok2->next()->isAssignmentOp()) {
                    return true;
                }
            }
...
void CheckCondition::checkBadBitmaskCheck()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        if (tok->str() == MatchCompiler::makeConstString("|") && tok->astOperand1() && tok->astOperand2() && tok->astParent()) {
            const Token* parent = tok->astParent();
            const bool isBoolean = match24(parent) ||
                                   (parent->str() == MatchCompiler::makeConstString("?") && parent->astOperand1() == tok) ||
                                   (parent->str() == MatchCompiler::makeConstString("=") && parent->astOperand2() == tok && parent->astOperand1() && parent->astOperand1()->variable() && match23(parent->astOperand1()->variable()->typeStartToken())) ||
                                   (parent->str() == MatchCompiler::makeConstString("(") && match25(parent->astOperand1())) ||
        ...
        for (const MathLib::bigint num1 : numbers) {
            ...
            if (match27(tok)) {
                if ((expr1->str() == MatchCompiler::makeConstString("&") && (num1 & num2) != num2) ||
                    ...
                    comparisonError(expr1, expr1->str(), num1, op, num2, op != MatchCompiler::makeConstString("=="));
                }
            } else if (expr1->str() == MatchCompiler::makeConstString("&")) {
                const bool or_equal = match28(tok);
                const std::string& op(tok->str());
                if ((match29(tok)) && (num1 < num2)) {
                    comparisonError(expr1, expr1->str(), num1, op, num2, !or_equal);
                } else if ((match30(tok)) && (num1 <= num2)) {
                    comparisonError(expr1, expr1->str(), num1, op, num2, or_equal);
                }
            } else if (expr1->str() == MatchCompiler::makeConstString("|")) {
...
bool CheckCondition::isOverlappingCond(const Token * const cond1, const Token * const cond2, bool pure) const
{
    ...
    if (cond1->str() == MatchCompiler::makeConstString("&") && cond1->astOperand1() && cond2->astOperand2()) {
        ...
        if (!match31(cond2) || !cond2->astOperand1() || !cond2->astOperand2())
            return false;
...
void CheckCondition::multiCondition2()
{
    ...
    for (const Scope &scope : symbolDatabase->scopeList) {
        ...
        if (!match17(scope.classDef->linkAt(1)))
            continue;
            ...
            if (cond->varId()) {
                ...
                if (!nonlocal && var) {
                    if (!(var->isLocal() || var->isArgument()))
                        nonlocal = true;
                    else if ((var->isPointer() || var->isReference()) && !match34(cond->astParent()))
                        // TODO: if var is pointer check what it points at
                        nonlocal = true;
                }
            } else if (!nonlocal && cond->isName()) {
                // varid is 0. this is possibly a nonlocal variable..
                nonlocal = match35(cond->astParent()) || match36(cond) || (cond->isCpp() && cond->str() == MatchCompiler::makeConstString("this"));
            } else {
        ...
        for (const MULTICONDITIONTYPE type:types) {
            ...
            for (; tok && tok != endToken; tok = tok->next()) {
                ...
                if (isExpressionChangedAt(cond1, tok, 0, false, *mSettings))
                    break;
                if (match38(tok)) {
                    const Token * condStartToken = tok->str() == MatchCompiler::makeConstString("if") ? tok->next() : tok;
                    const Token * condEndToken = tok->str() == MatchCompiler::makeConstString("if") ? condStartToken->link() : findmatch5(condStartToken) ;
                    // Does condition modify tracked variables?
                    if (findExpressionChanged(cond1, condStartToken, condEndToken, *mSettings))
                        break;

                    // Condition..
                    const Token *cond2 = tok->str() == MatchCompiler::makeConstString("if") ? condStartToken->astOperand2() : condStartToken->astOperand1();
                    const bool isReturnVar = (tok->str() == MatchCompiler::makeConstString("return") && (!match39(cond2) || (cond2 && cond2->isUnaryOp("!"))));
                ...
                if (match40(tok) && nonlocal && isNonConstFunctionCall(tok, mSettings->library)) // non const function call -> bailout if there are nonlocal variables
                    break;
                if (match41(tok) && tok->scope() == endToken->scope())
                    break;
                if (match42(tok))
                    break;
                ...
                if ((tok->varId() && vars.find(tok->varId()) != vars.end()) ||
                    ...
                    if (match46(tok))
                        break;
                    if (match47(tok->astParent())) {
                        const Token *parent = tok;
                        while (match48(parent->astParent()) || (parent->astParent() && parent->astParent()->isUnaryOp("*")))
                            parent = parent->astParent();
                        if (match49(parent->astParent()))
                            break;
                    }
                    if (tok->isCpp() && match50(tok) && (!tok->valueType() || !tok->valueType()->isIntegral()))
                        break;
                    ...
                    if (match56(tok->previous()) && isParameterChanged(tok))
                        break;
                }
            }
        }
...
static std::string innerSmtString(const Token * tok)
{
    ...
    if (top->str() == MatchCompiler::makeConstString("(") && top->astOperand1())
...
void CheckCondition::identicalConditionAfterEarlyExitError(const Token *cond1, const Token* cond2, ErrorPath errorPath)
{
    ...
    const bool isReturnValue = cond2 && match57(cond2->astParent());
...
static int sufficientCondition(std::string op1, const bool not1, const T value1, std::string op2, const bool not2, const T value2, const bool isAnd) {
    ...
    if (op1 == op2) {
        ...
        if (op1 == MatchCompiler::makeConstString("!="))
            res = 1;
        else if (op2 == MatchCompiler::makeConstString("!="))
            res = -1;
        else if (op1 == MatchCompiler::makeConstString("=="))
            res = -1;
        else if (op2 == MatchCompiler::makeConstString("=="))
            res = 1;
        else if (op1 == MatchCompiler::makeConstString(">") && op2 == MatchCompiler::makeConstString(">="))
...
static bool isIfConstexpr(const Token* tok) {
    ...
    return match59(top->astOperand1()) && top->astOperand1()->isConstexpr();
}
...
void CheckCondition::checkIncorrectLogicOperator()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        ...
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            if (!match60(tok) || !tok->astOperand1() || !tok->astOperand2())
                continue;
            ...
            const bool isLogicalOr(tok->str() == MatchCompiler::makeConstString("||"));
            ...
            const bool isAnd = tok->str() == MatchCompiler::makeConstString("&&");
...
void CheckCondition::checkModuloAlwaysTrueFalse()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (match62(tok->astOperand1()) && match61(tok->astOperand2())) {
                modulo = tok->astOperand1();
                num = tok->astOperand2();
            } else if (match61(tok->astOperand1()) && match62(tok->astOperand2())) {
                num = tok->astOperand1();
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            if (match63(tok)) {
                ...
                if (vt1 && vt1->type == ValueType::BOOL && !match65(tok->astOperand1()) && countPar(tok->astOperand1(), tok) == 0)
                    clarifyConditionError(tok, false, true);
                else if (vt2 && vt2->type == ValueType::BOOL && !match65(tok->astOperand2()) && countPar(tok, tok->astOperand2()) == 0)
                    clarifyConditionError(tok, false, true);
            }
        }
    }
...
void CheckCondition::alwaysTrueFalse()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (tok->link() && (match66(tok) ||
                                match67(tok->previous()))) {
                tok = tok->link();
                continue;
            }
            if (!tok->hasKnownIntValue())
                continue;
            if (match16(tok->previous()) && tok->previous()->function()) {
                const Function* f = tok->previous()->function();
                if (f->functionScope && match68(f->functionScope->bodyStart))
                    continue;
            }
            const Token* condition = nullptr;
            {
                // is this a condition..
                const Token *parent = tok->astParent();
                while (match60(parent))
                    parent = parent->astParent();
                if (!parent)
                    continue;
                if (parent->str() == MatchCompiler::makeConstString("?") && precedes(tok, parent))
                    condition = parent;
                else if (match18(parent->previous()))
                    condition = parent->previous();
                else if (match57(parent))
                    condition = parent;
                else if (parent->str() == MatchCompiler::makeConstString(";") && parent->astParent() && parent->astParent()->astParent() &&
                         match69(parent->astParent()->astParent()->previous()))
            ...
            if (!isUsedAsBool(tok, *mSettings))
                continue;
            if (match57(condition) && match71(tok))
                continue;
            if (match57(tok->astParent()) && match72(tok))
                continue;
            if (match73(tok))
                continue;
            if (match74(tok))
            ...
            if (match75(tok))
                continue;
            if (match16(tok->astOperand1()) && match57(tok->astParent()))
                continue;
            ...
            if (isConstVarExpression(tok, [](const Token* tok) {
                return match76(tok) && !match77(tok);
            }))
                continue;
...
void CheckCondition::alwaysTrueFalseError(const Token* tok, const Token* condition, const ValueFlow::Value* value)
{
    ...
    const std::string conditionStr = (match57(condition) ? "Return value" : "Condition");
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        for (const Token *lhs: lhsTokens) {
            ...
            for (const Token *expr: exprTokens) {
                if (lhs->str() == MatchCompiler::makeConstString("-") && expr == lhs->astOperand2())
                    continue; // TODO?
                ...
                if ((other->isNumber() && other->hasKnownIntValue() && other->getKnownIntValue() > 0) ||
                    ...
                    if (lhs->str() == MatchCompiler::makeConstString("+"))
                        result = (cmp == MatchCompiler::makeConstString(">") || cmp == MatchCompiler::makeConstString(">="));
                    else
                        result = (cmp == MatchCompiler::makeConstString("<") || cmp == MatchCompiler::makeConstString("<="));
                ...
                // x + y cmp x
                if (lhs->str() == MatchCompiler::makeConstString("+") && other->varId() > 0) {
...
void CheckCondition::invalidTestForOverflow(const Token* tok, const ValueType *valueType, const std::string &replace)
{
    ...
    if (replace == MatchCompiler::makeConstString("false") || replace == MatchCompiler::makeConstString("true"))
...
void CheckCondition::checkDuplicateConditionalAssign()
{
    ...
    for (const Scope *scope : symbolDatabase->functionScopes) {
        for (const Token *tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            if (!match81(tok))
                continue;
            if (!match17(tok->linkAt(1)))
                continue;
            const Token *blockTok = tok->linkAt(1)->next();
            const Token *condTok = tok->next()->astOperand2();
            const bool isBoolVar = match82(condTok);
            if (!isBoolVar && !match27(condTok))
                continue;
            if ((isBoolVar || condTok->str() == MatchCompiler::makeConstString("!=")) && match21(blockTok->link()))
                continue;
            if (!blockTok->next())
                continue;
            const Token *assignTok = blockTok->next()->astTop();
            if (!match83(assignTok))
                continue;
            if (nextAfterAstRightmostLeaf(assignTok) != blockTok->link()->previous())
                continue;
            bool isRedundant = false;
            if (isBoolVar) {
                const bool isNegation = condTok->str() == MatchCompiler::makeConstString("!");
...
void CheckCondition::checkAssignmentInCondition()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (match18(tok->astParent()->previous()))
                assignmentInCondition(tok);
            else if (match60(tok->astParent()))
                assignmentInCondition(tok);
            else if (match64(tok->astParent()) && tok == tok->astParent()->astOperand1())
                assignmentInCondition(tok);
        }
    }
}
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                if (kiv < typeMinValue || kiv > typeMaxValue) {
                    ...
                    case 0: // num cmp var
                        ...
                        else if (kiv == typeMaxValue && (tok->str() == MatchCompiler::makeConstString(">=") || tok->str() == MatchCompiler::makeConstString("<"))) {
                            error = true;
                        }
                        break;
                    case 1: // var cmp num
