...
#include <string>
#include <cstring>
#line 1 "/opt/cppcheck/cppcheck-main/lib/path.cpp"
/*
...
#undef CONST_CAST
    ...
    findAndReplace(marker, "Mode:", "");
    marker = trim(marker);
    if (marker == MatchCompiler::makeConstString("C++") || marker == MatchCompiler::makeConstString("c++")) {
...
Standards::Language Path::identify(const std::string &path, bool cppHeaderProbe, bool *header)
{
    ...
    if (ext == MatchCompiler::makeConstString(".C"))
        return Standards::Language::CPP;
    ...
    if (!caseInsensitiveFilesystem())
        strTolower(ext);
    if (ext == MatchCompiler::makeConstString(".h")) {
        if (header)
            *header = true;
