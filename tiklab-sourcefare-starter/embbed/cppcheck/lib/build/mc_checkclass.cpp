...
MAYBE_UNUSED static inline bool match21(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString(")")) || ((tok->tokType() == Token::eBracket || tok->tokType() == Token::eComparisonOp) && tok->str() == MatchCompiler::makeConstString(">")) || ((tok->tokType() == Token::eBracket) && tok->str() == MatchCompiler::makeConstString("}"))))
        return false;
    return true;
}
// pattern: >>|& %name%
...
MAYBE_UNUSED static inline bool match50(const Token* tok) {
    ...
    if (!tok || !(tok->str() == MatchCompiler::makeConstString(".")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->str() == MatchCompiler::makeConstString("clear")) || (tok->str() == MatchCompiler::makeConstString("Clear"))))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")))
        return false;
...
MAYBE_UNUSED static inline bool match52(const Token* tok) {
    if (!tok || !((tok->str() == MatchCompiler::makeConstString("try")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("do"))))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType() == Token::eBracket) && tok->str() == MatchCompiler::makeConstString("{")))
        return false;
...
// pattern: %varid% =|(|{
template<class T> MAYBE_UNUSED static inline T * findmatch55(T * start_tok, int varid) {
...
// pattern: ; __property ;
template<class T> MAYBE_UNUSED static inline T * findmatch56(T * start_tok, const Token * end) {
    ...
    if (!tok || !(tok->str() == MatchCompiler::makeConstString(";")))
        continue;
    tok = tok->next();
    if (!tok || !(tok->str() == MatchCompiler::makeConstString("__property")))
        continue;
    tok = tok->next();
    if (!tok || !(tok->str() == MatchCompiler::makeConstString(";")))
        continue;
...
MAYBE_UNUSED static inline bool match65(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eBitOp) && tok->str() == MatchCompiler::makeConstString("&")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("*")) || (tok->varId() != 0)))
        return false;
    return true;
}
// pattern: *
...
// pattern: %varid% =
MAYBE_UNUSED static inline bool match74(const Token* tok, const int varid) {
    if (varid==0U)
        throw InternalError(tok, "Internal error. Token::Match called with varid 0. Please report this to Cppcheck developers");
    if (!tok || !(tok->isName() && tok->varId() == varid))
        return false;
...
// pattern: this - %name%
template<class T> MAYBE_UNUSED static inline T * findmatch84(T * start_tok) {
    for (; start_tok; start_tok = start_tok->next()) {
...
MAYBE_UNUSED static inline bool match97(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")) || (tok->str() == MatchCompiler::makeConstString(".")) || (tok->str() == MatchCompiler::makeConstString("::"))))
        return false;
    return true;
}
// pattern: const_iterator|const_reverse_iterator
MAYBE_UNUSED static inline bool match98(const Token* tok) {
    if (!tok || !((tok->str() == MatchCompiler::makeConstString("const_iterator")) || (tok->str() == MatchCompiler::makeConstString("const_reverse_iterator"))))
        return false;
    return true;
}
// pattern: %cop%
...
// pattern: size|empty|cend|crend|cbegin|crbegin|max_size|length|count|capacity|get_allocator|c_str|str ( )
MAYBE_UNUSED static inline bool match100(const Token* tok) {
    if (!tok || !((tok->str() == MatchCompiler::makeConstString("size")) || (tok->str() == MatchCompiler::makeConstString("empty")) || (tok->str() == MatchCompiler::makeConstString("cend")) || (tok->str() == MatchCompiler::makeConstString("crend")) || (tok->str() == MatchCompiler::makeConstString("cbegin")) || (tok->str() == MatchCompiler::makeConstString("crbegin")) || (tok->str() == MatchCompiler::makeConstString("max_size")) || (tok->str() == MatchCompiler::makeConstString("length")) || (tok->str() == MatchCompiler::makeConstString("count")) || (tok->str() == MatchCompiler::makeConstString("capacity")) || (tok->str() == MatchCompiler::makeConstString("get_allocator")) || (tok->str() == MatchCompiler::makeConstString("c_str")) || (tok->str() == MatchCompiler::makeConstString("str"))))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")))
        return false;
...
MAYBE_UNUSED static inline bool match101(const Token* tok) {
    if (!tok || !((tok->str() == MatchCompiler::makeConstString("rfind")) || (tok->str() == MatchCompiler::makeConstString("copy"))))
        return false;
    return true;
}
// pattern: . %name% ( !!)
...
// pattern: ;
template<class T> MAYBE_UNUSED static inline T * findmatch115(T * start_tok) {
    for (; start_tok; start_tok = start_tok->next()) {
...
// pattern: %varid% = this|shared_from_this
MAYBE_UNUSED static inline bool match119(const Token* tok, const int varid) {
    if (varid==0U)
        throw InternalError(tok, "Internal error. Token::Match called with varid 0. Please report this to Cppcheck developers");
    if (!tok || !(tok->isName() && tok->varId() == varid))
        return false;
    ...
    if (!tok || !((tok->tokType() == Token::eAssignmentOp) && tok->str() == MatchCompiler::makeConstString("=")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->str() == MatchCompiler::makeConstString("this")) || (tok->str() == MatchCompiler::makeConstString("shared_from_this"))))
        return false;
    return true;
}
// pattern: %var% . reset ( )
...
// pattern: struct|class %name% :: %name%
...
#line 1 "/opt/cppcheck/cppcheck-main/lib/checkclass.cpp"
/*
...
void CheckClass::constructors()
{
    ...
    for (const Scope * scope : mSymbolDatabase->classAndStructScopes) {
        ...
        const bool unusedTemplate = match1(scope->classDef->previous());
        ...
        if (scope->numConstructors == 0 && printStyle && !usedInUnion) {
            ...
            if (needInit > haveInit) {
                if (haveInit == 0)
                    noConstructorError(scope->classDef, scope->className, scope->classDef->str() == MatchCompiler::makeConstString("struct"));
                else
        ...
        for (const Function &func : scope->functionList) {
            ...
            if (unusedTemplate && func.type == FunctionType::eConstructor) {
                ...
                if (match2(initList) && initList->next()->tokType() == Token::eName)
                    break;
            }
                ...
                if (!var.isPointer() && !var.isPointerArray() && var.isClass() && func.type == FunctionType::eConstructor) {
                    ...
                    if (!var.type()) {
                        if (var.isStlType() && var.valueType() && var.valueType()->containerTypeToken) {
                            ...
                            else if (var.getTypeName() == MatchCompiler::makeConstString("std::array")) {
...
{
    return type == MatchCompiler::makeConstString("std::initializer_list") || type == MatchCompiler::makeConstString("std::nullptr_t");
}
    ...
    for (const Scope * scope : mSymbolDatabase->classAndStructScopes) {
        ...
        for (const Function &func : scope->functionList) {
            ...
            if (!func.isExplicit() &&
                ...
                !(func.templateDef && match3(func.argumentList.front().typeEndToken())) &&
    ...
    for (const Scope * scope : mSymbolDatabase->classAndStructScopes) {
        ...
        for (const Function &func : scope->functionList) {
            if (func.type == FunctionType::eConstructor && func.functionScope) {
                ...
                for (const Token* const end = func.functionScope->bodyStart; tok != end; tok = tok->next()) {
                    if (match4(tok) ||
                        (match5(tok) && mSettings->library.getAllocFuncInfo(tok->tokAt(2)))) {
                ...
                for (const Token* const end = func.functionScope->bodyEnd; tok != end; tok = tok->next()) {
                    if (match6(tok) ||
                        (match7(tok) && mSettings->library.getAllocFuncInfo(tok->tokAt(2)))) {
                        const Variable* var = tok->variable();
                ...
                for (const Token* const end = func.functionScope->bodyEnd; tok != end; tok = tok->next()) {
                    if (match8(tok) ||
                        (match9(tok) && mSettings->library.getDeallocFuncInfo(tok))) {
                        const Token *vartok = tok->str() == MatchCompiler::makeConstString("delete") ? tok->next() : tok->tokAt(2);
        ...
        for (const Function &func : scope->functionList) {
            ...
            if (tok->str()==MatchCompiler::makeConstString(":")) {
                ...
                while (match11(tok)) {
                    if (allocatedVars.find(tok->varId()) != allocatedVars.end()) {
                        if (tok->varId() && match12(tok->tokAt(2)))
                            copiedVars.insert(tok);
                        else if (!match13(tok->tokAt(2)))
            ...
            for (tok = func.functionScope->bodyStart; tok != func.functionScope->bodyEnd; tok = tok->next()) {
                if ((tok->isCpp() && match6(tok)) ||
                    (match7(tok) && (mSettings->library.getAllocFuncInfo(tok->tokAt(2)) || mSettings->library.getReallocFuncInfo(tok->tokAt(2))))) {
                    allocatedVars.erase(tok->varId());
                } else if (match14(tok) && allocatedVars.find(tok->varId()) != allocatedVars.end()) {
                    copiedVars.insert(tok);
                }
            }
            break;
...
void CheckClass::initializeVarList(const Function &func, std::list<const Function *> &callstack, const Scope *scope, std::vector<Usage> &usage) const
{
    ...
    for (; ftok && ftok != func.functionScope->bodyEnd; ftok = ftok->next()) {
        ...
        if (initList) {
            if (level == 0 && match15(ftok) && match16(ftok->linkAt(1))) {
            ...
            } else if (level != 0 && match17(ftok)) // assignment in the initializer: var(value = x)
                assignVar(usage, ftok->varId());

            // Level handling
            if (ftok->link() && match18(ftok))
                level++;
            else if (ftok->str() == MatchCompiler::makeConstString("{")) {
                if (level != 0 ||
                    (match19(ftok->previous()) && match20(ftok->link())))
                    level++;
                else
                    initList = false;
            } else if (ftok->link() && match21(ftok))
                level--;
        }
        ...
        if (match22(ftok) && isLikelyStreamRead(ftok)) {
        ...
        if (match25(ftok->next())) {
            assignAllVar(usage);
            break;
        }

        // Using swap to assign all variables..
        if (func.type == FunctionType::eOperatorEqual && match26(ftok) && match27(ftok->linkAt(2))) {
            assignAllVar(usage);
            break;
        }

        // Calling member variable function?
        if (match28(ftok->next()) && !(ftok->next()->valueType() && ftok->next()->valueType()->pointer)) {
        ...
        // skip "return"
        if (ftok->str() == MatchCompiler::makeConstString("return"))
            ftok = ftok->next();
        ...
        if (match40(ftok)) {
            ...
            if (ftok->strAt(2) == MatchCompiler::makeConstString("&"))
                ++offsetToMember;
        ...
        } else if (match43(ftok) && !match44(ftok)) {
            if (ftok->str() == MatchCompiler::makeConstString("::"))
                ftok = ftok->next();

            // Passing "this" => assume that everything is initialized
            for (const Token *tok2 = ftok->linkAt(1); tok2 && tok2 != ftok; tok2 = tok2->previous()) {
                if (tok2->str() == MatchCompiler::makeConstString("this")) {
                    assignAllVar(usage);
                    return;
                }
            }
                    ...
                    for (const Token *tok2 = ftok; tok2; tok2 = tok2->next()) {
                        ...
                        if (match46(tok2)) {
                            ...
                            if (tok2->str() == MatchCompiler::makeConstString("&"))
                                tok2 = tok2->next();
                            if (isVariableChangedByFunctionCall(tok2, tok2->strAt(-1) == MatchCompiler::makeConstString("&"), tok2->varId(), *mSettings, nullptr))
        ...
        // Assignment of member variable?
            ...
            if (tok2->variable() && (bailout || tok2->variable()->isArray()) && tok2->strAt(1) != MatchCompiler::makeConstString("["))
                assignVar(usage, tok2->varId());
        }

        // Assignment of array item of member variable?
        else if (match47(ftok)) {
            const Token *tok2 = ftok;
            while (tok2) {
                if (tok2->strAt(1) == MatchCompiler::makeConstString("["))
                    tok2 = tok2->linkAt(1);
                else if (match48(tok2->next()))
                    tok2 = tok2->tokAt(2);
                else
                    break;
            }
        ...
        // Assignment of array item of member variable?
        else if (match31(ftok)) {
            assignVar(usage, ftok->next()->varId());
        } else if (match33(ftok)) {
            assignVar(usage, ftok->tokAt(2)->varId());
        } else if (match34(ftok)) {
            assignVar(usage, ftok->tokAt(2)->varId());
        } else if (match49(ftok)) {
...
void CheckClass::initializationListUsage()
{
    ...
    for (const Scope *scope : mSymbolDatabase->functionScopes) {
        ...
        if (const Token *initList = scope->function->constructorMemberInitialization()) {
            if (match51(initList) && initList->strAt(1) == scope->className)
                continue;
        }

        const Scope* owner = scope->functionOf;
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            if (match11(tok)) // Assignments might depend on this function call or if/for/while/switch statement from now on.
                break;
            if (match52(tok))
                break;
            if (!match53(tok) || tok->strAt(-1) == MatchCompiler::makeConstString("*") || tok->strAt(-1) == MatchCompiler::makeConstString("."))
                continue;
            ...
            visitAstNodes(tok->next()->astOperand2(),
                          [&](const Token *rhs) {
                if (rhs->str() == MatchCompiler::makeConstString(".") && rhs->astOperand1() && rhs->astOperand1()->variable() && rhs->astOperand1()->variable()->isLocal())
                    localmember = true;
            ...
            visitAstNodes(tok->next()->astOperand2(),
                ...
                if (var2) {
                    if (var2->scope() == owner && tok2->strAt(-1)!=MatchCompiler::makeConstString(".")) { // Is there a dependency between two member variables?
                        allowed = false;
                ...
                } else if (tok2->str() == MatchCompiler::makeConstString("this")) { // 'this' instance is not completely constructed in initialization list
                    allowed = false;
                    return ChildrenToVisit::done;
                } else if (match11(tok2) && tok2->strAt(-1) != MatchCompiler::makeConstString(".") && isMemberFunc(owner, tok2)) { // Member function called?
                    allowed = false;
                    return ChildrenToVisit::done;
                }
                return ChildrenToVisit::op1_and_op2;
...
static bool checkFunctionUsage(const Function *privfunc, const Scope* scope)
{
    ...
    for (auto func = scope->functionList.cbegin(); func != scope->functionList.cend(); ++func) {
        if (func->functionScope) {
            if (match11(func->tokenDef)) {
                for (const Token *ftok = func->tokenDef->tokAt(2); ftok && ftok->str() != MatchCompiler::makeConstString(")"); ftok = ftok->next()) {
                    if (match54(ftok) && ftok->strAt(1) == privfunc->name())
                        return true;
                    if (ftok->str() == MatchCompiler::makeConstString("("))
                        ftok = ftok->link();
                }
    ...
    for (const Variable &var : scope->varlist) {
        if (var.isStatic()) {
            const Token* tok = findmatch55(scope->bodyStart, var.declarationId()) ;
            if (tok)
                tok = tok->tokAt(2);
...
void CheckClass::privateFunctions()
{
    ...
    for (const Scope * scope : mSymbolDatabase->classAndStructScopes) {
        ...
        if (findmatch56(scope->bodyStart, scope->bodyEnd) )
            continue;
    ...
    for (const Scope *scope : mSymbolDatabase->functionScopes) {
        for (const Token *tok = scope->bodyStart; tok && tok != scope->bodyEnd; tok = tok->next()) {
            if (match57(tok)) {
                ...
                if (sizeofTok && sizeofTok->astOperand1() && match58(sizeofTok->astOperand1()->previous()))
                    sizeofTok = sizeofTok->astOperand1();
                else if (sizeofTok && sizeofTok->astOperand2() && match58(sizeofTok->astOperand2()->previous()))
                    sizeofTok = sizeofTok->astOperand2();
                if (match59(sizeofTok))
                    sizeofTok = sizeofTok->previous();
                if (match60(sizeofTok))
                    typeTok = sizeofTok->tokAt(2);
                else if (match61(sizeofTok))
                    typeTok = sizeofTok->tokAt(4);
                else if (match62(sizeofTok))
                    typeTok = sizeofTok->tokAt(3);
                else if (match63(sizeofTok) || match64(arg1)) {
                    type = findFunctionOf(sizeofTok->scope());
                } else if (match65(arg1)) {
                    int numIndirToVariableType = 0; // Offset to the actual type in terms of dereference/addressof
                    for (;; arg1 = arg1->next()) {
                        if (arg1->str() == MatchCompiler::makeConstString("&"))
                            ++numIndirToVariableType;
                        else if (arg1->str() == MatchCompiler::makeConstString("*"))
                            --numIndirToVariableType;
                        else
                            break;
                    }
                    ...
                    if (var && arg1->strAt(1) == MatchCompiler::makeConstString(",")) {
                        if (var->isArrayOrPointer()) {
                            const Token *endTok = var->typeEndToken();
                            while (match66(endTok)) {
                                ++numIndirToVariableType;
                        ...
                        if (!type && !var->isPointer() && !match67(var->typeStartToken()) &&
                ...
                if (typeTok && typeTok->str() == MatchCompiler::makeConstString("("))
            ...
            } else if (tok->variable() && tok->variable()->isPointer() && tok->variable()->typeScope() && match7(tok)) {
...
void CheckClass::checkMemsetType(const Scope *start, const Token *tok, const Scope *type, bool allocation, std::set<const Scope *> parsedTypes)
{
    ...
    for (const Variable &var : type->varlist) {
        ...
        if (!var.isStatic() && !var.isConst() && !var.isPointer() && (!var.isArray() || var.typeEndToken()->str() != MatchCompiler::makeConstString("*"))) {
            ...
            // check for std:: type
            if (var.isStlType() && typeName != MatchCompiler::makeConstString("std::array") && !mSettings->library.podtype(typeName)) {
                if (allocation)
            ...
            // check for float
            else if (printPortability && var.isFloatingType() && tok->str() == MatchCompiler::makeConstString("memset"))
    ...
    for (const Scope * scope : mSymbolDatabase->classAndStructScopes) {
        for (auto func = scope->functionList.cbegin(); func != scope->functionList.cend(); ++func) {
            if (func->type == FunctionType::eOperatorEqual && func->hasBody()) {
                ...
                if (func->retType == func->nestedIn->definedType && func->tokenDef->strAt(-1) == MatchCompiler::makeConstString("&")) {
...
void CheckClass::checkReturnPtrThis(const Scope *scope, const Function *func, const Token *tok, const Token *last, std::set<const Function*>& analyzedFunctions)
{
    ...
    for (; tok && tok != last; tok = tok->next()) {
        ...
        if (retExpr && retExpr->str() == MatchCompiler::makeConstString("="))
            retExpr = retExpr->astOperand1();
        if (retExpr && retExpr->isUnaryOp("*") && match69(retExpr->astOperand1()))
            continue;
        ...
        if (tok->strAt(2) == MatchCompiler::makeConstString("(") &&
            tok->linkAt(2)->strAt(1) == MatchCompiler::makeConstString(";")) {
            ...
            for (auto it = scope->functionList.cbegin(); it != scope->functionList.cend(); ++it) {
                ...
                if (it->type == FunctionType::eFunction && it->hasBody() &&
                    ...
                    if (it->tokenDef->strAt(-1) == MatchCompiler::makeConstString("&") &&
    ...
    for (const Scope * scope : mSymbolDatabase->classAndStructScopes) {
        ...
        for (const Function &func : scope->functionList) {
            if (func.type == FunctionType::eOperatorEqual && func.hasBody()) {
                ...
                while (typeTok->str() == MatchCompiler::makeConstString("const") || typeTok->str() == MatchCompiler::makeConstString("&") || typeTok->str() == MatchCompiler::makeConstString("*"))
                    typeTok = typeTok->previous();
                if (typeTok->str() != scope->className)
                    continue;

                // make sure return signature is correct
                if (match72(func.retDef) && func.retDef->str() == scope->className) {
...
{
    const Token *end;
    if (ifStatementScopeStart->str() == MatchCompiler::makeConstString("{"))
...
bool CheckClass::hasAllocation(const Function *func, const Scope* scope, const Token *start, const Token *end) const
{
    ...
    for (const Token *tok = start; tok && (tok != end); tok = tok->next()) {
        ...
        const Token *var;
        if (match9(tok) && mSettings->library.getDeallocFuncInfo(tok))
            var = tok->tokAt(2);
        else if (tok->isCpp() && match73(tok))
            var = tok->tokAt(3);
        else if (tok->isCpp() && match8(tok))
            var = tok->next();
        else
            continue;
...
{
    bool res = true;
    for (const Token *itr = tok; itr && itr->str()!=MatchCompiler::makeConstString("("); itr=itr->astParent()) {
        if (match75(itr) && (isTrueKeyword(itr->astOperand1()) || isTrueKeyword(itr->astOperand2()))) {
            res = !res;
        } else if (match75(itr) && ((match69(itr->astOperand1()) && match76(itr->astOperand2()) && Token::simpleMatch(itr->astOperand2()->next(), rhs->str().c_str(), rhs->str().size()))
                                                     || (match69(itr->astOperand2()) && match76(itr->astOperand1()) && Token::simpleMatch(itr->astOperand1()->next(), rhs->str().c_str(), rhs->str().size())))) {
            res = !res;
        } else if (match75(itr) && (isFalseKeyword(itr->astOperand1()) || isFalseKeyword(itr->astOperand2()))) {
            //Do nothing
        } else if (match77(itr)) {
            res = !res;
        } else if (match78(itr) && (isFalseKeyword(itr->astOperand1()) || isFalseKeyword(itr->astOperand2()))) {
            res = !res;
        } else if (match78(itr) && (isTrueKeyword(itr->astOperand1()) || isTrueKeyword(itr->astOperand2()))) {
            //Do nothing
        } else if (match78(itr) && ((match69(itr->astOperand1()) && match76(itr->astOperand2()) && Token::simpleMatch(itr->astOperand2()->next(), rhs->str().c_str(), rhs->str().size()))
                                                     || (match69(itr->astOperand2()) && match76(itr->astOperand1()) && Token::simpleMatch(itr->astOperand1()->next(), rhs->str().c_str(), rhs->str().size())))) {
            //Do nothing
        } else {
            return Bool::BAILOUT;
...
bool CheckClass::hasAssignSelf(const Function *func, const Token *rhs, const Token *&out_ifStatementScopeStart)
{
    ...
    for (const Token *tok = func->functionScope->bodyStart; tok && tok != last; tok = tok->next()) {
        ...
        visitAstNodes(tok->next()->astOperand2(),
            ...
            if (!match81(tok2))
                return ChildrenToVisit::op1_and_op2;
            if (match69(tok2->astOperand1()))
                tok2 = tok2->astOperand2();
            else if (match69(tok2->astOperand2()))
        ...
        for (const Function &func : scope->functionList) {
            ...
            auto isPointerOrReference = [this](const Token* start, const Token* end) -> bool {
                ...
                for (const Token* tok = start; tok != end; tok = tok->next()) {
                    if (tok->str() == MatchCompiler::makeConstString("{")) // end of trailing return type
                        return false;
                    if (tok->str() == MatchCompiler::makeConstString("<")) {
                        if (!tok->link())
                    ...
                    else if (!isConstTemplArg && match85(tok))
                        return true;
                }
                return false;
            };
            ...
            if (Function::returnsPointer(&func, /*unknown*/ true) || Function::returnsReference(&func, /*unknown*/ true, /*includeRValueRef*/ true)) { // returns const/non-const depending on template arg
                ...
                for (const Token* tok2 = func.retDef; precedes(tok2, func.token); tok2 = tok2->next())
                    if (tok2->isTemplateArg() && tok2->str() == MatchCompiler::makeConstString("const")) {
                        isTemplateArg = true;
                        break;
                    }
                if (isTemplateArg)
            ...
            if (func.tokenDef->str() == MatchCompiler::makeConstString("("))
                functionName += ")";
            else if (func.tokenDef->str() == MatchCompiler::makeConstString("["))
                functionName += "]";
...
static const Token* getFuncTokFromThis(const Token* tok) {
    ...
    while (match38(tok))
        tok = tok->tokAt(2);
    return match11(tok) ? tok : nullptr;
}
...
bool CheckClass::isMemberVar(const Scope *scope, const Token *tok) const
{
    ...
    do {
        ...
        if (tok->str() == MatchCompiler::makeConstString("this"))
            return !getFuncTokFromThis(tok); // function calls are handled elsewhere
        if (match87(tok->tokAt(-3)))
            return true;
        if (match88(tok->tokAt(-3))) {
            tok = tok->tokAt(-3);
            again = true;
        } else if (match89(tok->tokAt(-2))) {
            tok = tok->tokAt(-2);
            again = true;
        } else if (match90(tok->tokAt(-2))) {
    ...
    for (const Variable& var : scope->varlist) {
        if (var.name() == tok->str()) {
            ...
            while (match38(fqTok->tokAt(-2)))
                fqTok = fqTok->tokAt(-2);
            if (fqTok->strAt(-1) == MatchCompiler::makeConstString("::"))
    ...
    if (!tok->function()) {
        for (const Function &func : scope->functionList) {
            if (func.name() == tok->str()) {
                ...
                int argsPassed = tok2->str() == MatchCompiler::makeConstString(")") ? 0 : 1;
                for (;;) {
...
bool CheckClass::checkConstFunc(const Scope *scope, const Function *func, MemberAccess& memberAccessed) const
{
    ...
    auto getFuncTok = [](const Token* tok) -> const Token* {
        ...
        bool isReturn = false;
        if ((match91(tok) || (isReturn = match92(tok->astParent()))) && !tok->isStandardType() && !tok->isKeyword()) {
            if (isReturn)
                tok = tok->astParent();
            return tok;
        }
    ...
    auto checkFuncCall = [this, &memberAccessed](const Token* funcTok, const Scope* scope, const Function* func) {
        if (isMemberFunc(scope, funcTok) && (funcTok->strAt(-1) != MatchCompiler::makeConstString(".") || match36(funcTok->tokAt(-2)))) {
        ...
        if (const Function* f = funcTok->function()) { // check known function
            ...
            for (nonneg int argIndex = 0; argIndex < argMax; ++argIndex) {
                ...
                if (!argVar || ((argVar->isArrayOrPointer() || argVar->isReference()) &&
                    ...
                    while (match59(arg) && arg->astOperand2())
                        arg = arg->astOperand2();
    ...
    for (const Token *tok1 = func->functionScope->bodyStart; tok1 && tok1 != func->functionScope->bodyEnd; tok1 = tok1->next()) {
        if (tok1->isName() && isMemberVar(scope, tok1)) {
            ...
            if (tok1->str() == MatchCompiler::makeConstString("this")) {
                if (tok1->previous()->isAssignmentOp())
                    return false;
                if (match94(tok1->previous())) // call using ptr to member function TODO: check constness
                    return false;
                if (match66(tok1->astParent()) && tok1->astParent()->astParent() && tok1->astParent()->astParent()->isIncDecOp())
                    return false;
            }
            ...
            if (lhs->str() == MatchCompiler::makeConstString("(") && tok1->astParent() && tok1->astParent()->astParent())
                lhs = tok1->astParent()->astParent();
            else if (lhs->str() == MatchCompiler::makeConstString("?") && lhs->astParent())
                lhs = lhs->astParent();
            else if (lhs->str() == MatchCompiler::makeConstString(":") && lhs->astParent() && lhs->astParent()->astParent() && lhs->astParent()->str() == MatchCompiler::makeConstString("?"))
                lhs = lhs->astParent()->astParent();
            if (lhs->str() == MatchCompiler::makeConstString("&")) {
                const Token* parent = lhs->astParent();
                while (match95(parent)) {
                ...
                if (top->isAssignmentOp()) {
                    if (match96(top->astOperand2()) && !top->astOperand2()->previous()->function()) // TODO: check usage in init list
                        return false;
                    if (top->previous()->variable()) {
                        if (top->previous()->variable()->typeStartToken()->strAt(-1) != MatchCompiler::makeConstString("const") && top->previous()->variable()->isPointer())
                            return false;
                    }
                }
            } else if (lhs->str() == MatchCompiler::makeConstString(":") && lhs->astParent() && lhs->astParent()->str() == MatchCompiler::makeConstString("(")) { // range-based for-loop (C++11)
                ...
                if (lhs->astParent()->strAt(1) != MatchCompiler::makeConstString("const"))
                    return false;
            } else {
            ...
            for (;;) {
                if (match48(end->next())) {
                    ...
                    if (end->varId())
                        lastVarTok = end;
                } else if (end->strAt(1) == MatchCompiler::makeConstString("[")) {
                    if (end->varId()) {
                        ...
                        if (var && assignTok && assignTok->isAssignmentOp() && assignTok->astOperand1() && assignTok->astOperand1()->variable()) {
                            ...
                            if (assignVar->isPointer() && !assignVar->isConst() && var->typeScope()) {
                                ...
                                if (std::none_of(funcMap.cbegin(), funcMap.cend(), [](const std::pair<std::string, const Function*>& fm) {
                                    return fm.second->isConst() && fm.first == MatchCompiler::makeConstString("operator[]") && !Function::returnsConst(fm.second);
                                }))
                                    return false;
                            }
                        }
            ...
            auto hasOverloadedMemberAccess = [](const Token* end, const Scope* scope) -> bool {
                if (!end || !scope || !match86(end->astParent()))
                    return false;
            ...
            auto isConstContainerUsage = [&](const ValueType* vt) -> bool {
                ...
                if (contains({Library::Container::Yield::START_ITERATOR, Library::Container::Yield::END_ITERATOR, Library::Container::Yield::ITERATOR}, yield)) {
                    if (parent && parent->isComparisonOp())
                        return true;
                    // TODO: use AST
                    if (parent && parent->isAssignmentOp() && tok1->tokAt(-2)->variable() && match98(tok1->tokAt(-2)->variable()->typeEndToken()))
                        return true;
                }
                if ((yield == Library::Container::Yield::ITEM || yield == Library::Container::Yield::AT_INDEX) &&
                    ((parent && parent->isComparisonOp()) || lhs->isAssignmentOp() || (lhs->str() == MatchCompiler::makeConstString("(") && match99(lhs->astParent()))))
            ...
            if (end->strAt(1) == MatchCompiler::makeConstString("(")) {
                ...
                if (var->isStlType() // assume all std::*::size() and std::*::empty() are const
                    ...
                    // empty body
                } else if (var->isSmartPointer() && match86(tok1->next()) && tok1->next()->originalName().empty() && mSettings->library.isFunctionConst(end)) {
                    // empty body
            ...
            // Streaming
            else if (end->strAt(1) == MatchCompiler::makeConstString("<<") && tok1->strAt(-1) != MatchCompiler::makeConstString("<<"))
                return false;
            ...
            if (start->strAt(-1) == MatchCompiler::makeConstString("delete"))
                return false;

            tok1 = jumpBackToken?jumpBackToken:end; // Jump back to first [ to check inside, or jump to end of expression
            if (tok1 == end && match102(end->previous()) && !checkFuncCall(tok1, scope, func)) // function call on member
                return false;
        }

        // streaming: <<
        else if (match103(tok1->previous()) &&
        ...
        // streaming: >> *this
        else if (match104(tok1) && isLikelyStreamRead(tok1)) {
            return false;
        }
        ...
        // function/constructor call, return init list
            ...
            if (!checkFuncCall(funcTok, scope, func))
                return false;
        } else if (match106(tok1) && (!tok1->link() || !match105(tok1->link()->previous()))) {
            return false;
        }
    }
                            ...
                            for (; tok != end; tok = tok->next()) {
                                ...
                                if (const Variable* argVar = scope->getVariable(tok->str())) {
                                    ...
                                    if (tok->variable() && tok->variable()->isArgument())
                                        continue;
                                    if (var->isPointer() && (argVar->isArray() || match76(tok->astParent())))
                                        continue;
                                    if (var->isReference())
                                        continue;
                                    if (match108(tok->astParent()))
                                        continue;
                                                                     ...
                                                                     std::map<const Function *, std::list<const Token *>> & virtualFunctionCallsMap)
{
    ...
    for (const Token *tok = function.arg->link(); tok != function.functionScope->bodyEnd; tok = tok->next()) {
        if (function.type != FunctionType::eConstructor &&
            ...
            if ((match79(tok) && tok->link() && match111(tok->link()->previous())) ||
                match112(tok)) {
        ...
        if (!callFunction ||
            ...
            match86(tok->previous()) ||
            !(tok->astParent() && (tok->astParent()->str() == MatchCompiler::makeConstString("(") || (tok->astParent()->str() == MatchCompiler::makeConstString("::") && match59(tok->astParent()->astParent())))))
            continue;
        ...
        if (callFunction->isImplicitlyVirtual()) {
            if (!callFunction->isPure() && match113(tok->previous()))
                continue;
    ...
    const std::string &funcname)
{
    ...
    if (scopeFunction) {
        ...
        for (const Token *tok = scopeFunction->tokenDef; tok != endToken; tok = tok->next()) {
            if (!constructorName.empty() && match114(tok->previous()))
                constructorName += ' ';
...
static const Token* getSingleFunctionCall(const Scope* scope) {
    ...
    const Token* const end = findmatch115(start) ;
    ...
    if (match59(ftok) && ftok->previous()->function())
...
static const Variable* getSingleReturnVar(const Scope* scope) {
    ...
    if (!end || end->next() != scope->bodyEnd)
        return nullptr;
    if (!start->astOperand1() || start->str() != MatchCompiler::makeConstString("return"))
        return nullptr;
            ...
            if (!var.isStatic()) {
                ...
                for (const Function &func : classScope->functionList) {
                    ...
                    for (const Token *tok = func.functionScope->bodyStart; tok != func.functionScope->bodyEnd; tok = tok->next()) {
                        if (match119(tok, var.declarationId())) {
                            hasAssign = true;
                            break;
                        }
                    }
...
bool CheckClass::checkThisUseAfterFreeRecursive(const Scope *classScope, const Function *func, const Variable *selfPointer, std::set<const Function *> callstack, const Token *&freeToken)
{
    ...
    for (const Token *tok = bodyStart; tok != bodyEnd; tok = tok->next()) {
        ...
        if (match83(tok) && selfPointer == tok->next()->variable()) {
            freeToken = tok;
            tok = tok->tokAt(2);
        } else if (match120(tok) && selfPointer == tok->variable())
            freeToken = tok;
        else if (match121(tok->previous()) && tok->function() && tok->function()->nestedIn == classScope && tok->function()->type == FunctionType::eFunction) {
            if (isDestroyed) {
            ...
            if (checkThisUseAfterFreeRecursive(classScope, tok->function(), selfPointer, callstack, freeToken))
                return true;
        } else if (isDestroyed && match122(tok->previous()) && tok->variable() && tok->variable()->scope() == classScope && !tok->variable()->isStatic() && !tok->variable()->isArgument()) {
            thisUseAfterFree(selfPointer->nameToken(), freeToken, tok);
            return true;
        } else if (freeToken && match123(tok)) {
            // TODO
            return tok->str() == MatchCompiler::makeConstString("throw");
        } else if (tok->str() == MatchCompiler::makeConstString("{") && tok->scope()->type == ScopeType::eLambda) {
            tok = tok->link();
        }
    }
    return false;
...
void CheckClass::checkUnsafeClassRefMember()
{
    ...
    for (const Scope * classScope : mSymbolDatabase->classAndStructScopes) {
        for (const Function &func : classScope->functionList) {
            ...
            const Token *initList = func.constructorMemberInitialization();
            while (match124(initList)) {
                if (match125(initList->tokAt(2))) {
...
Check::FileInfo *CheckClass::getFileInfo(const Tokenizer &tokenizer, const Settings& /*settings*/, const std::string& currentConfig) const
{
    ...
    for (const Scope * classScope : tokenizer.getSymbolDatabase()->classAndStructScopes) {
        ...
        if (classScope->classDef && match1(classScope->classDef->previous()))
            continue;
