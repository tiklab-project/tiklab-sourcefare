...
#include <string>
#include <cstring>
#line 1 "/opt/cppcheck/cppcheck-main/lib/suppressions.cpp"
/*
...
bool SuppressionList::isSuppressed(const SuppressionList::ErrorMessage &errmsg, bool global)
{
    ...
    const bool unmatchedSuppression(errmsg.errorId == MatchCompiler::makeConstString("unmatchedSuppression"));
...
bool SuppressionList::reportUnmatchedSuppressions(const std::list<SuppressionList::Suppression> &unmatched, ErrorLogger &errorLogger)
{
    ...
    for (const SuppressionList::Suppression &s : unmatched) {
        ...
        if (s.errorId == MatchCompiler::makeConstString("unmatchedSuppression"))
            continue;

        // check if this unmatched suppression is suppressed
        bool suppressed = false;
        for (const SuppressionList::Suppression &s2 : unmatched) {
            if (s2.errorId == MatchCompiler::makeConstString("unmatchedSuppression")) {
                if ((s2.fileName.empty() || s2.fileName == MatchCompiler::makeConstString("*") || s2.fileName == s.fileName) &&
