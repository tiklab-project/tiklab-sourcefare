...
MAYBE_UNUSED static inline bool match1(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("asm")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("goto"))))
        return false;
    return true;
}
...
MAYBE_UNUSED static inline bool match3(const Token* tok) {
    if (!tok || !((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("continue")))
        return false;
    return true;
}
...
// pattern: goto
template<class T> MAYBE_UNUSED static inline T * findmatch7(T * start_tok, const Token * end) {
...
// pattern: goto|break
template<class T> MAYBE_UNUSED static inline T * findmatch8(T * start_tok, const Token * end) {
    for (; start_tok && start_tok != end; start_tok = start_tok->next()) {
...
MAYBE_UNUSED static inline bool match26(const Token* tok) {
    if (!tok || !(tok->isName() || ((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString(")")) || ((tok->tokType() == Token::eExtendedOp || tok->tokType() == Token::eLambda) && tok->str() == MatchCompiler::makeConstString("]")) || ((tok->tokType() == Token::eBracket || tok->tokType() == Token::eComparisonOp) && tok->str() == MatchCompiler::makeConstString(">"))))
        return false;
    return true;
}
// pattern: %name%|::|<|.
MAYBE_UNUSED static inline bool match27(const Token* tok) {
    if (!tok || !(tok->isName() || (tok->str() == MatchCompiler::makeConstString("::")) || ((tok->tokType() == Token::eBracket || tok->tokType() == Token::eComparisonOp) && tok->str() == MatchCompiler::makeConstString("<")) || (tok->str() == MatchCompiler::makeConstString("."))))
        return false;
    return true;
}
// pattern: <
...
#line 1 "/opt/cppcheck/cppcheck-main/lib/forwardanalyzer.cpp"
/*
    ...
    struct ForwardTraversal {
        ...
        Progress traverseTok(T* tok, const F &f, bool traverseUnknown, T** out = nullptr) {
            ...
            if (match3(tok)) {
                ...
                return Break(Analyzer::Terminate::Escape);
            } else if (match5(tok) && isEscapeFunction(tok, settings.library)) {
                // Traverse the parameters of the function before escaping
                traverseRecursive(tok->next()->astOperand2(), f, traverseUnknown);
                return Break(Analyzer::Terminate::Escape);
            ...
            } else if (tok->astOperand1() && tok->astOperand2() && match6(tok)) {
                ...
                // Skip class scope
            } else if (tok->str() == MatchCompiler::makeConstString("{") && tok->scope() && tok->scope()->isClassOrStruct()) {
                if (out)
                    *out = tok->link();
            } else {
        ...
        Progress traverseConditional(T* tok, F f, bool traverseUnknown) {
            ...
            if (action.isNone() && match6(tok) && tok->astOperand1() && tok->astOperand2()) {
                ...
                if (childTok->str() == MatchCompiler::makeConstString(":")) {
                    ...
                    if (!checkThen && tok->str() == MatchCompiler::makeConstString("&&"))
                        return Progress::Continue;
                    if (!checkElse && tok->str() == MatchCompiler::makeConstString("||"))
                        return Progress::Continue;
        ...
        static bool hasGoto(const Token* endBlock) {
            return findmatch7(endBlock->link(), endBlock) ;
        }

        static bool hasJump(const Token* endBlock) {
            return findmatch8(endBlock->link(), endBlock) ;
        }
        ...
        Progress updateInnerLoop(Token* endBlock, Token* stepTok, Token* condTok) {
            ...
            if (stepTok && updateRecursive(stepTok) == Progress::Break)
                return Break();
            if (condTok && !match10(condTok) && updateRecursive(condTok) == Progress::Break)
                return Break();
            return Progress::Continue;
        }
        ...
        Progress updateRange(Token* start, const Token* end, int depth = 20) {
            ...
            for (Token* tok = start; precedes(tok, end); tok = tok->next()) {
                ...
                if (tok->link()) {
                    // Skip casts..
                    if (tok->str() == MatchCompiler::makeConstString("(") && !tok->astOperand2() && tok->isCast()) {
                        tok = tok->link();
                        continue;
                    }
                ...
                if (Token* assignTok = assignExpr(tok)) {
                    ...
                    if (match12(top->previous()) && match13(top->link())) {
                        ...
                        if (tok->astOperand2() != condTok && !match14(tok->astOperand2()))
                            return Break(Analyzer::Terminate::Bail);
                    ...
                    if (!tok)
                        return Break();
                } else if (!tok->variable() && (match15(tok) || tok->str() == MatchCompiler::makeConstString("case"))) {
                    if (!analyzer->lowerToPossible())
                        return Break(Analyzer::Terminate::Bail);
                } else if (tok->link() && tok->str() == MatchCompiler::makeConstString("}") && tok == tok->scope()->bodyEnd) { // might be an init list
                    ...
                    if (contains({ScopeType::eDo, ScopeType::eFor, ScopeType::eWhile, ScopeType::eIf, ScopeType::eElse, ScopeType::eSwitch}, scope->type)) {
                        ...
                        assert(!inDoWhile || match16(tok));
                        if (match17(tok) || inDoWhile)
                            tok = tok->linkAt(2);
                ...
                } else if (tok->isControlFlowKeyword() && match18(tok) &&
                           match13(tok->linkAt(1))) {
                    ...
                    if (match12(tok)) {
                        ...
                        if (!thenBranch.check && !elseBranch.check && stopOnCondition(condTok) && stopUpdates())
                            return Break(Analyzer::Terminate::Conditional);
                        const bool hasElse = match17(endBlock);
                        bool bail = false;
                    ...
                    Token* endBlock = tok->linkAt(1);
                    Token* condTok = match16(endBlock) ? endBlock->tokAt(2)->astOperand2() : nullptr;
