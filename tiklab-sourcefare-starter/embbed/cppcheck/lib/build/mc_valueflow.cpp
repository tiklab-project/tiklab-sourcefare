...
template<class T> MAYBE_UNUSED static inline T * findmatch5(T * start_tok, const Token * end) {
...
MAYBE_UNUSED static inline bool match8(const Token* tok) {
    if (!tok || !(tok->isName() || (tok->str() == MatchCompiler::makeConstString("::")) || ((tok->tokType() == Token::eBracket || tok->tokType() == Token::eComparisonOp) && tok->str() == MatchCompiler::makeConstString("<")) || ((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("*")) || ((tok->tokType() == Token::eBitOp) && tok->str() == MatchCompiler::makeConstString("&")) || ((tok->tokType() == Token::eLogicalOp) && tok->str() == MatchCompiler::makeConstString("&&"))))
        return false;
    return true;
}
// pattern: decltype (
...
// pattern: const|volatile|&|*|&&|%type%|::
MAYBE_UNUSED static inline bool match13(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("const")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("volatile")) || ((tok->tokType() == Token::eBitOp) && tok->str() == MatchCompiler::makeConstString("&")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("*")) || ((tok->tokType() == Token::eLogicalOp) && tok->str() == MatchCompiler::makeConstString("&&")) || (tok->isName() && tok->varId() == 0U) || (tok->str() == MatchCompiler::makeConstString("::"))))
        return false;
    return true;
}
// pattern: %name% :: %name%
...
MAYBE_UNUSED static inline bool match18(const Token* tok) {
    ...
    if (!tok || !(tok->str() == MatchCompiler::makeConstString("::")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->str() == MatchCompiler::makeConstString("value")))
        return false;
    return true;
}
// pattern: %var% [ %num%| ] = {
...
// pattern: %varid% . %name% (
MAYBE_UNUSED static inline bool match79(const Token* tok, const int varid) {
    if (varid==0U)
...
// pattern: break|goto
template<class T> MAYBE_UNUSED static inline T * findmatch100(T * start_tok, const Token * end) {
    for (; start_tok && start_tok != end; start_tok = start_tok->next()) {
...
// pattern: %varid%
template<class T> MAYBE_UNUSED static inline T * findmatch128(T * start_tok, const Token * end, int varid) {
    for (; start_tok && start_tok != end; start_tok = start_tok->next()) {
...
template<class T> MAYBE_UNUSED static inline T * findmatch136(T * start_tok, const Token * end) {
    for (; start_tok && start_tok != end; start_tok = start_tok->next()) {
...
// pattern: union
template<class T> MAYBE_UNUSED static inline T * findmatch139(T * start_tok, const Token * end) {
    for (; start_tok && start_tok != end; start_tok = start_tok->next()) {
...
#line 1 "/opt/cppcheck/cppcheck-main/lib/valueflow.cpp"
/*
...
{
    while (match8(tok) && !tok->isVariable()) {
        if (tok->link())
            tok = tok->link();
        tok = tok->next();
    }
...
static std::vector<const Token*> evaluateType(const Token* start, const Token* end)
{
    ...
    for (const Token* tok = start; tok != end; tok = tok->next()) {
        ...
        if (match9(tok)) {
            ...
            } else if (match11(tok->next()) && match12(tok->linkAt(3))) {
...
static void valueFlowTypeTraits(TokenList& tokenlist, const Settings& settings)
{
    ...
    eval["is_void"] = [&](std::vector<std::vector<const Token*>> args) {
        ...
        stripCV(args[0]);
        if (args[0].size() == 1 && args[0][0]->str() == MatchCompiler::makeConstString("void"))
    ...
    eval["is_lvalue_reference"] = [&](const std::vector<std::vector<const Token*>>& args) {
        if (args.size() != 1)
            return ValueFlow::Value::unknown();
        if (args[0].back()->str() == MatchCompiler::makeConstString("&"))
            return ValueFlow::Value(1);
        if (!hasUnknownType(args[0]))
            return ValueFlow::Value(0);
        return ValueFlow::Value::unknown();
    ...
    for (Token* tok = tokenlist.front(); tok; tok = tok->next()) {
        ...
        if (endsWith(traitName, "_v")) {
            ...
            traitName.pop_back();
            updateTok = tok->next();
        } else if (match17(templateTok->link()) ||
                   match18(templateTok->link())) {
    ...
    for (Token* tok = tokenlist.front(); tok; tok = tok->next()) {
        if (tok->varId() > 0) {
            ...
            // const array decl
                     ...
                     tok->variable()->nameToken() == tok && match19(tok)) {
            ...
            // pointer = array
            else if (tok->variable() && tok->variable()->isArray() && match20(tok->astParent()) &&
...
static void valueFlowArrayBool(TokenList& tokenlist, const Settings& settings)
{
    for (Token* tok = tokenlist.front(); tok; tok = tok->next()) {
        ...
        if (!var->isArray() || var->isArgument() || var->isStlType())
            continue;
        if (isNonZero(getOtherOperand(tok)) && match23(tok->astParent()))
            continue;
        // TODO: Check for function argument
        if ((astIsBool(tok->astParent()) && !match24(tok->astParent())) ||
            (tok->astParent() && match25(tok->astParent()->previous()))) {
...
static void valueFlowArrayElement(TokenList& tokenlist, const Settings& settings)
{
    for (Token* tok = tokenlist.front(); tok; tok = tok->next()) {
        ...
        if (match26(tok) && tok->isBinaryOp()) {
            indexTok = tok->astOperand2();
            arrayTok = tok->astOperand1();
        } else if (match27(tok->tokAt(-2)) && astIsContainer(tok->tokAt(-2)->astOperand1())) {
...
static std::vector<MathLib::bigint> minUnsignedValue(const Token* tok, int depth = 8)
{
    ...
    if (const ValueFlow::Value* v = tok->getKnownValue(ValueFlow::Value::ValueType::INT)) {
        result = {v->intvalue};
    } else if (!match30(tok) && tok->isConstOp() && tok->astOperand1() && tok->astOperand2()) {
...
static void valueFlowImpossibleValues(TokenList& tokenList, const Settings& settings)
{
    for (Token* tok = tokenList.front(); tok; tok = tok->next()) {
        ...
        if (match32(tok) && match31(tok->astOperand1())) {
        ...
        } else if (match33(tok) && tok->astOperand2() && tok->astOperand2()->hasKnownIntValue()) {
            ...
            value.setImpossible();
            setTokenValue(tok->next(), std::move(value), settings);
        } else if (match35(tok) && astIsContainerOwned(tok->astOperand1())) {
        ...
        } else if (match36(tok) && match37(tok->linkAt(1))) {
            ValueFlow::Value value{0};
            value.setImpossible();
            setTokenValue(tok->linkAt(1)->next(), std::move(value), settings);
        } else if ((tokenList.isCPP() && match38(tok)) || tok->isUnaryOp("&")) {
            ValueFlow::Value value{0};
            value.setImpossible();
            setTokenValue(tok, std::move(value), settings);
...
static void valueFlowGlobalConstVar(TokenList& tokenList, const Settings& settings)
{
    ...
    for (const Token* tok = tokenList.front(); tok; tok = tok->next()) {
        ...
        if (tok == tok->variable()->nameToken() && !tok->variable()->isVolatile() && !tok->variable()->isArgument() &&
            ...
            tok->valueType()->pointer == 0 && tok->valueType()->constness == 1 && match39(tok) &&
...
static void valueFlowGlobalStaticVar(TokenList& tokenList, const Settings& settings)
{
    ...
    for (const Token* tok = tokenList.front(); tok; tok = tok->next()) {
        ...
        if (tok == tok->variable()->nameToken() && tok->variable()->isStatic() && !tok->variable()->isConst() &&
            ...
            tok->valueType()->constness == 0 && match39(tok) && tok->next()->astOperand2() &&
            ...
            if (!tok->astParent())
                continue;
            if (match40(tok->astParent()) && !tok->astParent()->astOperand2())
                vars.erase(tok->variable());
            else if (tok->astParent()->isAssignmentOp()) {
                if (tok == tok->astParent()->astOperand1())
                    vars.erase(tok->variable());
                else if (tok->isCpp() && match41(tok->astParent()->tokAt(-2)))
                    vars.erase(tok->variable());
            } else if (isLikelyStreamRead(tok->astParent())) {
                vars.erase(tok->variable());
            } else if (match42(tok->astParent()))
                vars.erase(tok->variable());
        }
    }
...
static bool isConditionKnown(const Token* tok, bool then)
{
    ...
    if (match25(top->previous()))
        return parent == top || match43(parent);
...
static std::string lifetimeType(const Token *tok, const ValueFlow::Value *val)
{
    ...
    case ValueFlow::Value::LifetimeKind::Address:
        if (astIsPointer(tok))
            return "pointer";
        if (match20(tok) && astIsPointer(tok->astOperand2()))
            return "pointer";
        return "object";
    }
                                                               ...
                                                               int depth = 20)
{
    ...
    if (var && var->declarationId() == tok->varId()) {
        if (var->isReference() || var->isRValueReference()) {
            ...
            } else if (match46(var->nameToken()->astParent()) &&
                       var->nameToken()->astParent()->astParent() &&
                       match47(var->nameToken()->astParent()->astParent()->previous())) {
        ...
        if (f) {
            ...
            for (const Token* returnTok : returns) {
                ...
                for (ValueFlow::LifetimeToken& lt : getLifetimeTokens(returnTok, escape, errorPath, pred, settings, depth - returns.size())) {
                    ...
                    } else if (match27(tok->tokAt(-2)) && !derefShared(tok->tokAt(-2)) &&
        ...
        if (match27(tok->tokAt(-2)) && tok->tokAt(-2)->originalName() != MatchCompiler::makeConstString("->") && astIsContainer(tok->tokAt(-2)->astOperand1())) {
    ...
    } else if (match49(tok) || tok->isUnaryOp("*")) {
        ...
        if (derefShared(vartok->astParent())) {
            ...
            return ValueFlow::LifetimeToken::setAddressOf(
               ...
               !match51(getArgumentStart(tok)) && getArgumentStart(tok)->valueType()) {
...
static bool isDifferentType(const Token* src, const Token* dst, const Settings& settings)
{
    ...
    if (t && parentT) {
        ...
        if (isNotEqual(decl, parentdecl) && !(isCpp && (match53(decl.first) || match53(parentdecl.first))))
            return true;
...
static bool isRangeForScope(const Scope* scope)
{
    ...
    if (!scope->bodyStart)
        return false;
    if (!match54(scope->bodyStart->previous()))
        return false;
    return match46(scope->bodyStart->linkAt(-1)->astOperand2());
}
    ...
    visitAstNodes(tok, [&](const Token* child) {
        if (const Variable* var = child->variable()) {
            ...
            if (var->isLocal() || var->isArgument()) {
                ...
                if (match55(top->tokAt(-1))) { // variable declared in if (...)
                    const Token* elseTok = top->link()->linkAt(1);
                    if (match56(elseTok) && tok->scope()->isNestedIn(elseTok->tokAt(2)->scope()))
...
static void valueFlowForwardLifetime(Token * tok, const TokenList &tokenlist, ErrorLogger &errorLogger, const Settings &settings)
{
    ...
    if (match57(tok->previous()) && isVariableDecl(tok->previous())) {
    ...
    if (!parent)
        return;
    // Assignment
    if (parent->str() == MatchCompiler::makeConstString("=") && (!parent->astParent() || match43(parent->astParent()))) {
        // Rhs values..
        ...
        // Function call
    } else if (match48(parent->previous())) {
...
static void valueFlowLifetimeFunction(Token *tok, const TokenList &tokenlist, ErrorLogger &errorLogger, const Settings &settings)
{
    ...
    if (match27(tok->astParent()) && astIsRHS(tok))
                                              ...
                                              const Settings& settings)
{
    ...
    if (!t) {
        if (tok->valueType() && tok->valueType()->type != ValueType::RECORD)
            return;
        if (tok->str() != MatchCompiler::makeConstString("{") && !match63(tok->previous()) && !isVariableDecl(tok->previous()))
            return;
...
static void valueFlowLifetimeConstructor(Token* tok, const TokenList& tokenlist, ErrorLogger& errorLogger, const Settings& settings)
{
    ...
    if (tok->valueType()) {
        vts = {*tok->valueType()};
    } else if (match57(tok->previous()) && isVariableDecl(tok->previous()) &&
               tok->previous()->valueType()) {
        vts = {*tok->previous()->valueType()};
    } else if (match4(tok) && !match64(tok->previous())) {
...
struct Lambda {
    ...
    {
        if (!match26(tok) || !tok->link())
            return;
        capture = tok;

        if (match65(capture->link())) {
        ...
        for (const Token* c:getCaptures()) {
            ...
            } else if (c->isUnaryOp("&") && match69(c->astOperand1())) {
...
static bool isDecayedPointer(const Token *tok)
{
    ...
    if (!tok->astParent())
        return false;
    if (match20(tok->astParent()) && astIsRHS(tok))
        return true;
    if (astIsPointer(tok->astParent()) && !match70(tok->astParent()))
        return true;
    if (tok->astParent()->isConstOp())
        return true;
    if (!match70(tok->astParent()))
        return false;
...
static void valueFlowLifetime(TokenList &tokenlist, ErrorLogger &errorLogger, const Settings &settings)
{
    for (Token *tok = tokenlist.front(); tok; tok = tok->next()) {
        ...
        // address of
            ...
            if (match71(tok->astParent()))
                continue;
            if (match26(tok->astOperand1())) {
            ...
            for (const ValueFlow::LifetimeToken& lt : ValueFlow::getLifetimeTokens(tok->astOperand1(), settings)) {
                ...
                if (lt.addressOf || astIsPointer(lt.token) || !match72(lt.token->astParent()))
        ...
        // container lifetimes
            ...
            } else if (astIsIterator(parent) && match48(parent->previous()) &&
                       contains({Library::Container::Yield::START_ITERATOR, Library::Container::Yield::END_ITERATOR},
                                astFunctionYield(parent->previous(), settings))) {
        ...
        // Check constructors
        else if (match74(tok) && !isScope(tok->next())) {
            valueFlowLifetimeConstructor(tok->next(), tokenlist, errorLogger, settings);
        }
        // Check function calls
        else if (match48(tok) && !match54(tok->linkAt(1))) {
            valueFlowLifetimeFunction(tok, tokenlist, errorLogger, settings);
        }
        // Unique pointer lifetimes
        else if (astIsUniqueSmartPointer(tok) && astIsLHS(tok) && match75(tok->astParent())) {
...
static Token* findEndOfFunctionCallForParameter(Token* parameterToken)
{
    ...
    while (parent && !parent->isOp() && !match80(parent))
        parent = parent->astParent();
    if (!parent)
        return nullptr;
...
static void valueFlowAfterMove(const TokenList& tokenlist, const SymbolDatabase& symboldatabase, ErrorLogger& errorLogger, const Settings& settings)
{
    ...
    for (const Scope * scope : symboldatabase.functionScopes) {
        ...
        for (auto* tok = const_cast<Token*>(start); tok != scope->bodyEnd; tok = tok->next()) {
            Token * varTok;
            if (match81(tok) && tok->next()->originalName().empty()) {
                varTok = tok;
            ...
            if (endOfFunctionCall) {
                if (endOfFunctionCall->str() == MatchCompiler::makeConstString(")")) {
                    ...
                    while (match58(ternaryColon))
                        ternaryColon = ternaryColon->astParent();
                    if (match46(ternaryColon)) {
                        endOfFunctionCall = ternaryColon->astOperand2();
                        if (match58(endOfFunctionCall))
    ...
    if (tok->str() == op) {
        ...
        std::copy_if(args.cbegin(), args.cend(), std::back_inserter(conds), [&](const Token* tok2) {
            ...
            if (tok2->hasKnownIntValue())
                return false;
            if (match82(tok2) && !astIsBool(tok2))
                return false;
            return true;
        });
    }
...
static bool isBreakOrContinueScope(const Token* endToken)
{
    if (!match83(endToken))
        return false;
    return match84(endToken->tokAt(-2));
}
        ...
        for (auto* tok = const_cast<Token*>(scope->bodyStart); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            // Inner condition
            {
                for (const Token* condTok2 : getConditions(condTok, "&&")) {
                    if (is1) {
                        const bool isBool = astIsBool(condTok2) || match85(condTok2);
    ...
    for (const Scope* scope : symboldatabase.functionScopes) {
        for (auto* tok = const_cast<Token*>(scope->bodyStart); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (match34(tok)) {
                ...
                if (vartok->exprId() == 0)
                    continue;
                if (match88(tok) && !astIsLHS(vartok))
                    continue;
                if (match89(tok) && constant->intvalue != 0)
                    continue;
                if (match90(tok) && constant->intvalue != 1)
                    continue;
                                            ...
                                            const Settings& settings)
{
    ...
    while ((next = findExpressionChangedSkipDeadCode(
        ...
        if (match92(next->astParent()))
            b = ValueFlow::Value::Bound::Lower;
        else if (match93(next->astParent()))
                                   ...
                                   const Settings& settings)
{
    ...
    if (tokenlist.isCPP() && vars.size() == 1 && match95(vars.front()->typeStartToken())) {
        for (ValueFlow::Value& value : values) {
            if (value.isImpossible())
                continue;
            if (value.isIntValue())
                               ...
                               const Settings& settings)
{
    for (const Scope* scope : symboldatabase.functionScopes) {
        for (auto* tok = const_cast<Token*>(scope->bodyStart); tok != scope->bodyEnd; tok = tok->next()) {
            if (!match97(tok))
                continue;
            if (!match51(tok->next()->astOperand2()))
                continue;
...
static bool isBreakScope(const Token* const endToken)
{
    ...
    if (!match4(endToken->link()))
        return false;
    return findmatch100(endToken->link(), endToken) ;
}
...
struct ConditionHandler {
    struct Condition {
                                   ...
                                   bool known = false) const
        {
            ...
            if (!match101(condTok) && condTok != vartok) {
                           ...
                           const std::function<void(const Condition& cond, Token* tok, const Scope* scope)>& f) const
    {
        for (const Scope *scope : symboldatabase.functionScopes) {
            ...
            for (auto *tok = const_cast<Token *>(scope->bodyStart); tok != scope->bodyEnd; tok = tok->next()) {
                ...
                if (!match25(top->previous()) && !match104(tok->astParent()))
                    continue;
    ...
    void beforeCondition(TokenList& tokenlist,
        ...
        traverseCondition(symboldatabase, settings, skippedFunctions, [&](const Condition& cond, Token* tok, const Scope*) {
            ...
            if (match105(top))
                return;
            if (match106(cond.vartok->astParent()))
                return;

            if (match32(tok->astParent()) && tok->astParent()->isExpandedMacro()) {
                if (settings.debugwarnings)
                    bailout(tokenlist,
                            errorLogger,
                            tok,
            ...
            // if,macro => bailout
            if (match55(top->previous()) && top->previous()->isExpandedMacro()) {
                if (settings.debugwarnings)
                    bailout(tokenlist,
                            errorLogger,
                            tok,
            ...
            if (match108(top->previous()) && match54(top->link())) {

                // Variable changed in 3rd for-expression
                if (match47(top->previous())) {
                ...
                if (findExpressionChanged(cond.vartok, start, end, settings)) {
                    ...
                    if (!match106(tok) &&
                        ...
                        return match105(tok2->astParent()) && astIsLHS(tok2);
    ...
    static Token* skipNotAndCasts(Token* tok, bool* inverted = nullptr)
    {
        for (; tok->astParent(); tok = tok->astParent()) {
            if (match109(tok->astParent())) {
                if (inverted)
                    *inverted ^= true;
                continue;
            }
            if (match110(tok->astParent())) {
                ...
                if (sibling->hasKnownIntValue() && (astIsBool(tok) || astIsBool(sibling))) {
                    ...
                    if (inverted)
                        *inverted ^= value == match111(tok->astParent());
                    continue;
                }
            }
        ...
        traverseCondition(symboldatabase, settings, skippedFunctions, [&](const Condition& cond, Token* condTok, const Scope* scope) {
            ...
            if (match112(ctx->astParent())) {
                ...
                if (parent) {
                    ...
                    if (allowKnown && (match110(condTok) || cond.isBool()))
                        changePossibleToKnown(values);
            ...
            if (top->strAt(-1) == MatchCompiler::makeConstString("for")) {
                ...
                if (match92(stepTok) && bounds.count(ValueFlow::Value::Bound::Lower) > 0)
                    return;
                if (match93(stepTok) && bounds.count(ValueFlow::Value::Bound::Upper) > 0)
                    return;
            ...
            if (changeBlock >= 0 && !match114(top->previous())) {
                if (settings.debugwarnings)
                    bailout(tokenlist,
                            errorLogger,
            ...
            if (match54(top->link())) {
                ...
                if (condTok->astParent() && match115(top->previous()))
                ...
                if (dead_if || dead_else) {
                    ...
                    if (match116(parent)) {
                        ...
                        while (parent && parent->str() == op)
                            parent = parent->astParent();
                        if (match109(parent) || match117(parent))
...
struct SimpleConditionHandler : ConditionHandler {
    std::vector<Condition> parse(const Token* tok, const Settings& /*settings*/) const override {
        ...
        } else if (tok->astParent() && (match119(tok->astParent()) ||
                                        match120(tok->astParent()->previous()))) {
            if (match20(tok))
                vartok = tok->astOperand1();
...
static bool isIntegralOrPointer(const Token* tok)
{
    ...
    if (isIntegralOnlyOperator(tok->astParent()))
        return true;
    if (match124(tok) && tok->isBinaryOp())
...
static void valueFlowInferCondition(TokenList& tokenlist, const Settings& settings)
{
    for (Token* tok = tokenlist.front(); tok; tok = tok->next()) {
        ...
        if (tok->hasKnownIntValue())
            continue;
        if (match125(tok) && tok->astOperand1() && tok->astOperand2()) {
...
struct SymbolicConditionHandler : SimpleConditionHandler {
    ...
    std::vector<Condition> parse(const Token* tok, const Settings& settings) const override
    {
        ...
        if (match110(tok) && (isNegatedBool(tok->astOperand1()) || isNegatedBool(tok->astOperand2()))) {
                                     ...
                                     const Settings& settings)
{
    ...
    for (Token *tok2 = bodyStart->next(); tok2 != bodyEnd; tok2 = tok2->next()) {
        ...
        if (match54(tok2)) {
            if (vartok->varId() && findmatch128(tok2->link(), tok2, vartok->varId()) ) {
                if (settings.debugwarnings)
...
static void valueFlowForLoop(TokenList &tokenlist, const SymbolDatabase& symboldatabase, ErrorLogger &errorLogger, const Settings &settings)
{
    for (const Scope &scope : symboldatabase.scopeList) {
        ...
        if (!match43(tok->next()->astOperand2()) ||
            !match43(tok->next()->astOperand2()->astOperand2()))
            continue;
        ...
        if (extractForLoopValues(tok, varid, knownInitValue, initValue, partialCond, stepValue, lastValue)) {
            ...
            const Token* vartok = findmatch128(tok, bodyStart, varid) ;
        ...
        for (const Token* tok = scope.bodyStart->next(); tok != scope.bodyEnd; tok = tok->next()) {
            ...
            if (match130(tok)) {
                ...
                values.back().errorPath.emplace_back(tok,
                                                     ...
                                                     tok->strAt(1) + " here.");
                bool known = false;
                if ((match4(tok->previous()) || match131(tok->tokAt(-2))) &&
                    !match132(tok->tokAt(3)))
                    known = true;
                while (match133(tok->tokAt(3))) {
                    known = false;
                    tok = tok->tokAt(3);
                    if (!tok->isName())
                        tok = tok->next();
...
static std::list<ValueFlow::Value> getFunctionArgumentValues(const Token* argtok)
{
    ...
    if (argvalues.empty() && match134(argtok)) {
...
static void valueFlowFunctionDefaultParameter(const TokenList& tokenlist, const SymbolDatabase& symboldatabase, ErrorLogger& errorLogger, const Settings& settings)
{
    ...
    for (const Scope* scope : symboldatabase.functionScopes) {
        ...
        for (nonneg int arg = function->minArgCount(); arg < function->argCount(); arg++) {
            ...
            if (var && var->hasDefault() && match135(var->nameToken())) {
...
static Token* findStartToken(const Variable* var, Token* start, const Library& library)
{
    ...
    if (findmatch136(start, first) )
        return start;
    if (first != var->nameToken()) {
        // if this is lhs in assignment then set first to the first token in LHS expression
        Token* temp = first;
        while (match137(temp->astParent()) && precedes(temp->astParent(), temp))
            temp = temp->astParent();
        if (match20(temp->astParent()) && precedes(temp, temp->astParent()))
            first = temp;
    }
    ...
    if (scope == var->scope()) {
        ...
        for (const Token* parent = first; parent; parent = parent->astParent()) {
            if (match43(parent->astParent()) &&
                match43(parent->astParent()->astParent()) &&
                match58(parent->astParent()->astParent()->astParent()) &&
                match47(parent->astParent()->astParent()->astParent()->astOperand1()) &&
    ...
    if (!tok)
        return start;
    if (match56(tok->tokAt(-2)))
        tok = tok->linkAt(-2);
    if (match54(tok->previous()))
...
static void valueFlowUninit(TokenList& tokenlist, ErrorLogger& errorLogger, const Settings& settings)
{
    for (Token *tok = tokenlist.front(); tok; tok = tok->next()) {
        ...
        if (const Scope* scope = var->typeScope()) {
            if (findmatch139(scope->bodyStart, scope->bodyEnd) )
                continue;
                                       ...
                                       ValueFlow::Value& value)
{
    ...
    if (!value.isIntValue() && !value.isIteratorValue() && !value.isSymbolicValue())
        return expr;
    if (value.isSymbolicValue() && !match141(expr))
        return expr;
    ...
    if (value.isSymbolicValue() && rhs && match142(expr))
        return expr;
...
bool ValueFlow::isContainerSizeChanged(const Token* tok, int indirect, const Settings& settings, int depth)
{
    ...
    if (!tok->valueType() || !tok->valueType()->container)
        return true;
    if (astIsLHS(tok) && match143(tok->astParent()))
        return true;
    if (astIsLHS(tok) && match26(tok->astParent()))
    ...
    case Library::Container::Action::NO_ACTION:
        ...
        if (astIsLHS(tok) && match27(tok->astParent())) {
...
static void valueFlowSmartPointer(TokenList &tokenlist, ErrorLogger & errorLogger, const Settings &settings)
{
    for (Token *tok = tokenlist.front(); tok; tok = tok->next()) {
        ...
        if (!astIsSmartPointer(tok))
            continue;
        if (tok->variable() && match144(tok)) {
            const Variable* var = tok->variable();
            if (!var->isSmartPointer())
                continue;
            if (var->nameToken() == tok) {
                if (match96(tok) && tok->next()->astOperand2() &&
...
static void valueFlowContainerSetTokValue(const TokenList& tokenlist, ErrorLogger& errorLogger, const Settings& settings, const Token* tok, Token* initList)
{
    ...
    if (astIsContainerString(tok) && match4(initList) && match152(initList->astOperand2())) {
                                   ...
                                   const std::set<const Scope*>& skippedFunctions)
{
    ...
    for (const Variable *var : symboldatabase.variableList()) {
        ...
        if (var->valueType()->container->size_templateArgNo >= 0) {
            ...
            if (var->dimensions().size() == 1) {
                ...
                if (dim.known) {
                    size = dim.num;
                } else if (dim.tok && dim.tok->hasKnownIntValue() && dim.tok->strAt(1) == MatchCompiler::makeConstString(">")) {
        ...
        if (nameToken->hasKnownValue(ValueFlow::Value::ValueType::CONTAINER_SIZE))
            continue;
        if (!staticSize) {
            if (!match153(nameToken) &&
                !(match154(nameToken) && match155(nameToken->linkAt(1))) &&
                !match48(nameToken))
                continue;
        }
        if (match156(nameToken->astTop()->previous()))
        ...
        if (!staticSize) {
            ...
            } else if (match58(nameToken->next())) {
    ...
    for (const Scope *functionScope : symboldatabase.functionScopes) {
        for (auto* tok = const_cast<Token*>(functionScope->bodyStart); tok != functionScope->bodyEnd; tok = tok->next()) {
            if (match157(tok)) {
            ...
            } else if (match158(tok->previous()) && astIsContainer(tok) && getLibraryContainer(tok)->size_templateArgNo < 0) {
                std::vector<ValueFlow::Value> values;
                if (match4(tok)) {
                ...
                for (const ValueFlow::Value& value : values)
                    setTokenValue(tok, value, settings);
            } else if (match159(tok) && match80(tok->tokAt(2)->astOperand2()) &&
                       // init list
                       ((tok->tokAt(2) == tok->tokAt(2)->astOperand2()->astParent() && !tok->tokAt(2)->astOperand2()->astOperand2() && tok->tokAt(2)->astOperand2()->str() == MatchCompiler::makeConstString("{")) ||
                        // constructor
                        (!match50(tok->tokAt(2)->astOperand2()->astOperand1()) && settings.library.detectContainer(tok->tokAt(3))))) {
                Token* containerTok = tok->next();
                if (containerTok->exprId() == 0)
                    continue;
            ...
            } else if (match27(tok) && tok->astOperand1() && tok->astOperand1()->valueType() &&
            ...
            } else if (tok->str() == MatchCompiler::makeConstString("=") && match151(tok->astOperand2()) && astIsContainerString(tok)) {
                const Token* tok2 = tok->astOperand2();
                MathLib::bigint size = 0;
                while (match151(tok2) && tok2->astOperand2()) {
...
struct ContainerConditionHandler : ConditionHandler {
    std::vector<Condition> parse(const Token* tok, const Settings& settings) const override
    {
        ...
        if (match110(tok)) {
            ...
            if (match152(tok->astOperand1())) {
                strtok = tok->astOperand1();
                vartok = tok->astOperand2();
            } else if (match152(tok->astOperand2())) {
...
static void valueFlowDynamicBufferSize(const TokenList& tokenlist, const SymbolDatabase& symboldatabase, ErrorLogger& errorLogger, const Settings& settings)
{
    ...
    auto getBufferSizeFromNew = [&](const Token* newTok) -> MathLib::bigint {
        ...
        if (newTok && newTok->astOperand1()) { // number of elements
            ...
            else if (match62(newTok->astOperand1())) {
