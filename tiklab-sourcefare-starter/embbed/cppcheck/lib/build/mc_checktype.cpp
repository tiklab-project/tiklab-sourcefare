...
#line 1 "/opt/cppcheck/cppcheck-main/lib/checktype.cpp"
/*
...
void CheckType::checkTooBigBitwiseShift()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        // C++ and macro: OUT(x<<y)
        if (tok->isCpp() && match1(tok) && match2(tok->linkAt(2)) && tok->next()->isUpperCaseName() && !tok->next()->function())
            tok = tok->linkAt(2);
...
void CheckType::checkIntegerOverflow()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (tok->str() == MatchCompiler::makeConstString("<<") && value->intvalue > 0 && value->intvalue < (static_cast<MathLib::biguint>(1) << bits))
            continue;
...
void CheckType::checkSignConversion()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        if (!tok->isArithmeticalOp() || match4(tok))
            continue;
...
void CheckType::checkLongCast()
{
    ...
    // Assignments..
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        if (tok->str() != MatchCompiler::makeConstString("=") || !match5(tok->astOperand2()) || tok->astOperand2()->isUnaryOp("*"))
            continue;
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        ...
        if (!match6(def) || !def->next()->valueType())
            continue;
...
void CheckType::checkFloatToIntegerOverflow()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        // Explicit cast
        if (match8(tok) && tok->astOperand1() && !tok->astOperand2()) {
        ...
        // Assignment
        else if (tok->str() == MatchCompiler::makeConstString("=") && tok->astOperand1() && tok->astOperand2()) {
            if (isUnreachableOperand(tok))
                continue;
        ...
        else if (tok->str() == MatchCompiler::makeConstString("return") && tok->astOperand1() && tok->astOperand1()->valueType() && tok->astOperand1()->valueType()->isFloat()) {
            if (isUnreachableOperand(tok))
                continue;
