...
MAYBE_UNUSED static inline bool match5(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eComparisonOp) && tok->str() == MatchCompiler::makeConstString("==")) || ((tok->tokType() == Token::eComparisonOp) && tok->str() == MatchCompiler::makeConstString(">=")) || ((tok->tokType() == Token::eComparisonOp) && tok->str() == MatchCompiler::makeConstString("<=")) || ((tok->tokType() == Token::eBracket || tok->tokType() == Token::eComparisonOp) && tok->str() == MatchCompiler::makeConstString("<")) || ((tok->tokType() == Token::eBracket || tok->tokType() == Token::eComparisonOp) && tok->str() == MatchCompiler::makeConstString(">")) || ((tok->tokType() == Token::eComparisonOp) && tok->str() == MatchCompiler::makeConstString("!="))))
        return false;
    return true;
}
// pattern: !
...
MAYBE_UNUSED static inline bool match14(const Token* tok) {
    if (!tok || !((tok->str() == MatchCompiler::makeConstString(".")) || ((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")) || ((tok->tokType() == Token::eExtendedOp || tok->tokType() == Token::eLambda) && tok->str() == MatchCompiler::makeConstString("[")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("*")) || (tok->varId() != 0)))
        return false;
    return true;
}
// pattern: do|; {
MAYBE_UNUSED static inline bool match15(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("do")) || (tok->str() == MatchCompiler::makeConstString(";"))))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType() == Token::eBracket) && tok->str() == MatchCompiler::makeConstString("{")))
        return false;
...
MAYBE_UNUSED static inline bool match18(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("for")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("while")) || ((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("if"))))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")))
        return false;
...
MAYBE_UNUSED static inline bool match22(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eLogicalOp) && tok->str() == MatchCompiler::makeConstString("!")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("+")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("-"))))
        return false;
    return true;
}
// pattern: >|%name% {|(
...
#line 1 "/opt/cppcheck/cppcheck-main/lib/programmemory.cpp"
/*
...
static bool isBasicForLoop(const Token* tok)
{
    ...
    if (!start)
        return false;
    if (!match3(start->previous()))
        return false;
    if (!match4(start->astOperand2()))
        return false;
    return true;
}
...
static void programMemoryParseCondition(ProgramMemory& pm, const Token* tok, const Token* endTok, const Settings& settings, bool then)
{
    ...
    if (match5(tok)) {
        ...
        if (endTok && findExpressionChanged(vartok, tok->next(), endTok, settings))
            return;
        const bool impossible = (tok->str() == MatchCompiler::makeConstString("==") && !then) || (tok->str() == MatchCompiler::makeConstString("!=") && then);
...
static void fillProgramMemoryFromAssignments(ProgramMemory& pm, const Token* tok, const Settings& settings, const ProgramMemory& state, const ProgramMemory::Map& vars)
{
    ...
    for (const Token *tok2 = tok; tok2; tok2 = tok2->previous()) {
        if ((match11(tok2) || match10(tok2->previous())) && tok2->astOperand1() &&
            tok2->astOperand2()) {
            bool setvar = false;
        ...
        } else if (match12(tok2) && tok2->link() &&
                   match13(tok2->link()->previous())) {
        ...
        } else if (tok2->exprId() > 0 && match14(tok2) && !pm.hasValue(tok2->exprId()) &&
        ...
        if (tok2->str() == MatchCompiler::makeConstString("{")) {
            if (indentlevel <= 0) {
                ...
                if (!match15(tok2->previous()) && !conditionIsTrue(cond, state, settings) &&
                    (cond || !isBasicForLoop(tok2)))
                    break;
            } else
                --indentlevel;
            if (match16(tok2->previous()))
                tok2 = tok2->linkAt(-2)->previous();
        }
        if (tok2->str() == MatchCompiler::makeConstString("}") && !match17(tok2->link()->previous())) {
            const Token *cond = getCondTokFromEnd(tok2);
            const bool inElse = match16(tok2->link()->previous());
            if (cond) {
                if (conditionIsFalse(cond, state, settings)) {
                    if (inElse) {
                        ++indentlevel;
...
void ProgramMemoryState::assume(const Token* tok, bool b, bool isEmpty)
{
    ...
    if (match18(top->previous()) && !match19(tok->astParent())) {
...
static ValueFlow::Value evaluate(const std::string& op, const ValueFlow::Value& lhs, const ValueFlow::Value& rhs)
{
    ...
    if (error)
        return ValueFlow::Value::unknown();
    if (result.isImpossible() && op == MatchCompiler::makeConstString("!=")) {
        if (isTrue(result)) {
            result.intvalue = 1;
...
namespace {
    struct Executor {
        ...
        ValueFlow::Value executeImpl(const Token* expr)
        {
            ...
            if (!expr)
                return unknown();
            if (expr->hasKnownIntValue() && !expr->isAssignmentOp() && expr->str() != MatchCompiler::makeConstString(","))
            ...
            if (expr->isBoolean())
                return ValueFlow::Value{expr->str() == MatchCompiler::makeConstString("true")};
            if (match20(expr->tokAt(-2)) && astIsContainer(expr->tokAt(-2)->astOperand1())) {
                ...
                if (!lhs.isIntValue())
                    return unknown();
                // overflow
                if (!lhs.isImpossible() && lhs.intvalue == 0 && expr->str() == MatchCompiler::makeConstString("--") && astIsUnsigned(expr->astOperand1()))
                    return unknown();
            ...
            } else if (match21(expr) && expr->astOperand1() && expr->astOperand2()) {
                ValueFlow::Value lhs = execute(expr->astOperand1());
            ...
            // Unary ops
            else if (match22(expr) && expr->astOperand1() && !expr->astOperand2()) {
                ValueFlow::Value lhs = execute(expr->astOperand1());
                if (!lhs.isIntValue())
                    return unknown();
                if (expr->str() == MatchCompiler::makeConstString("!")) {
                ...
                return unknown();
            } else if (expr->str() == MatchCompiler::makeConstString("(") && expr->isCast()) {
                if (expr->astOperand2()) {
                    if (expr->astOperand1()->str() != MatchCompiler::makeConstString("dynamic_cast"))
                        return execute(expr->astOperand2());
                    return unknown();
                }
        ...
        std::vector<ValueFlow::Value> execute(const Scope* scope)
        {
            ...
            for (const Token* tok = scope->bodyStart->next(); precedes(tok, scope->bodyEnd); tok = tok->next()) {
                ...
                if (match24(top) && top->astOperand1())
