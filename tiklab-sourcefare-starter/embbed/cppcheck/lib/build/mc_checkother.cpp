...
MAYBE_UNUSED static inline bool match19(const Token* tok) {
    ...
    if (tok && (((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("("))))
        tok = tok->next();
    if (!tok || !(tok->isName() || tok->isBoolean() || (tok->tokType() == Token::eChar) || (tok->tokType() == Token::eString) || ((tok->tokType() == Token::eBitOp) && tok->str() == MatchCompiler::makeConstString("&"))))
        return false;
    return true;
}
// pattern: %type%
...
// pattern: ;
template<class T> MAYBE_UNUSED static inline T * findmatch41(T * start_tok) {
...
// pattern: [}:]
template<class T> MAYBE_UNUSED static inline T * findmatch53(T * start_tok) {
    for (; start_tok; start_tok = start_tok->next()) {
...
// pattern: {
template<class T> MAYBE_UNUSED static inline T * findmatch56(T * start_tok) {
    for (; start_tok; start_tok = start_tok->next()) {
...
// pattern: ; %varid% =
MAYBE_UNUSED static inline bool match64(const Token* tok, const int varid) {
    if (!tok || !(tok->str() == MatchCompiler::makeConstString(";")))
        return false;
    tok = tok->next();
    if (varid==0U)
...
// pattern: %varid% =
MAYBE_UNUSED static inline bool match71(const Token* tok, const int varid) {
    if (varid==0U)
        throw InternalError(tok, "Internal error. Token::Match called with varid 0. Please report this to Cppcheck developers");
    if (!tok || !(tok->isName() && tok->varId() == varid))
        return false;
...
// pattern: %varid% !!=
MAYBE_UNUSED static inline bool match72(const Token* tok, const int varid) {
    if (varid==0U)
        throw InternalError(tok, "Internal error. Token::Match called with varid 0. Please report this to Cppcheck developers");
    if (!tok || !(tok->isName() && tok->varId() == varid))
        return false;
...
// pattern: & %varid%
MAYBE_UNUSED static inline bool match73(const Token* tok, const int varid) {
    if (!tok || !((tok->tokType() == Token::eBitOp) && tok->str() == MatchCompiler::makeConstString("&")))
        return false;
    tok = tok->next();
    if (varid==0U)
...
// pattern: %varid%
template<class T> MAYBE_UNUSED static inline T * findmatch74(T * start_tok, const Token * end, int varid) {
...
// pattern: = %varid%
MAYBE_UNUSED static inline bool match76(const Token* tok, const int varid) {
    if (!tok || !((tok->tokType() == Token::eAssignmentOp) && tok->str() == MatchCompiler::makeConstString("=")))
        return false;
    tok = tok->next();
    if (varid==0U)
...
// pattern: & %var% = %varid%
MAYBE_UNUSED static inline bool match80(const Token* tok, const int varid) {
    if (!tok || !((tok->tokType() == Token::eBitOp) && tok->str() == MatchCompiler::makeConstString("&")))
        return false;
    tok = tok->next();
    if (!tok || !(tok->varId() != 0))
...
// pattern: sizeof|alignof|noexcept|typeid (
MAYBE_UNUSED static inline bool match99(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eKeyword) && tok->str() == MatchCompiler::makeConstString("sizeof")) || (tok->str() == MatchCompiler::makeConstString("alignof")) || (tok->str() == MatchCompiler::makeConstString("noexcept")) || (tok->str() == MatchCompiler::makeConstString("typeid"))))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")))
        return false;
...
// pattern: %var%
template<class T> MAYBE_UNUSED static inline T * findmatch134(T * start_tok, const Token * end) {
    for (; start_tok && start_tok != end; start_tok = start_tok->next()) {
...
#line 1 "/opt/cppcheck/cppcheck-main/lib/checkother.cpp"
/*
...
void CheckOther::checkUnreachableCode()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        ...
        for (const Token* tok = scope->bodyStart; tok && tok != scope->bodyEnd; tok = tok->next()) {
            ...
            while (match49(secondBreak) && secondBreak->scope()->type == ScopeType::eUnconditional)
            ...
            if (secondBreak && secondBreak->scope()->nestedIn && secondBreak->scope()->nestedIn->type == ScopeType::eSwitch &&
                ...
                while (match50(secondBreak) && secondBreak->scope()->type == ScopeType::eUnconditional)
                    secondBreak = secondBreak->next();
            }
...
void CheckOther::checkVariableScope()
{
    ...
    for (const Variable* var : symbolDatabase->variableList()) {
        ...
        if (isStructuredBindingVariable(var) && // warn for single decomposition
            !(match59(var->nameToken()->astParent()) && var->nameToken()->astParent()->astOperand2() == var->nameToken()))
            continue;
        ...
        if (match64(tok, var->declarationId())) { // bailout for assignment
            tok = tok->tokAt(2)->astOperand2();
            if (!isSimpleExpr(tok, var, *mSettings))
                continue;
        }
        else if (match65(tok)) { // bailout for constructor
            isConstructor = true;
        ...
        if (!isConstructor && match67(tok) && match68(tok->astOperand2()))
            continue;
        bool reduce = true;
        bool used = false; // Don't warn about unused variables
        for (; tok && tok != var->scope()->bodyEnd; tok = tok->next()) {
            if (tok->str() == MatchCompiler::makeConstString("{") && tok->scope() != tok->previous()->scope() && !tok->isExpandedMacro() && !isWithinScope(tok, var, ScopeType::eLambda)) {
                if (used) {
                    bool used2 = false;
            ...
            } else if (match69(tok) && match70(tok->linkAt(3))) {
                tok = tok->next();
...
bool CheckOther::checkInnerScope(const Token *tok, const Variable* var, bool& used) const
{
    ...
    if (scope->type == ScopeType::eUnconditional && (tok->strAt(-1) == MatchCompiler::makeConstString(")") || tok->previous()->isName())) // Might be an unknown macro like BOOST_FOREACH
        loopVariable = true;
    ...
    for (; tok && tok != end; tok = tok->next()) {
        ...
        if (loopVariable && noContinue && tok->scope() == scope && !forHeadEnd && scope->type != ScopeType::eSwitch && match71(tok, var->declarationId())) { // Assigned in outer scope.
            loopVariable = false;
        ...
        if (loopVariable && match72(tok, var->declarationId())) // Variable used in loop
            return false;

        if (match73(tok, var->declarationId())) // Taking address of variable
            return false;

        if (match71(tok, var->declarationId())) {
            if (!bFirstAssignment && var->isInit() && findmatch74(tok->tokAt(2), findmatch41(tok->tokAt(3)) , var->declarationId()) )
                return false;
        ...
        if (match76(tok, var->declarationId()) && (var->isArray() || var->isPointer() || (var->valueType() && var->valueType()->container))) // Create a copy of array/pointer. Bailout, because the memory it points to might be necessary in outer scope
            return false;
...
void CheckOther::checkPassByReference()
{
    ...
    for (const Variable* var : symbolDatabase->variableList()) {
        ...
        if (var->isArray() && (!var->isStlType() || match59(var->nameToken()->next())))
            continue;
    ...
    for (const Variable *var : symbolDatabase->variableList()) {
        ...
        // Skip if another non-const variable is initialized with this variable
        {
            ...
            for (const Token* tok = var->nameToken(); tok != scope->bodyEnd && tok != nullptr; tok = tok->next()) {
                ...
                if (tok->isUnaryOp("&") && match73(tok, var->declarationId())) {
...
void CheckOther::checkConstPointer()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (tok == nameTok) {
            ...
            if (var->isLocal() && (!var->isStatic() || match59(nameTok->next())) &&
        ...
        if (parent && (parent->isUnaryOp("*") || (((hasIncDecPlus = parent->isIncDecOp()) || (hasIncDecPlus = (parent->str() == MatchCompiler::makeConstString("+")))) &&
            ...
            deref = DEREF;
        else if (match59(parent) && parent->astOperand1() == tok && tok != nameTok)
            deref = DEREF;
        ...
        if (deref != NONE) {
            ...
            while (match59(gparent) && parent != gparent->astOperand2() && parent->str() == gparent->str())
            ...
            if (match48(gparent)) {
                ...
                if (!takingRef && !nonConstPtrAssignment)
                    continue;
            } else if (match59(gparent) && gparent->astOperand2() == parent)
                continue;
            ...
            if (match86(parent))
                continue;
            if (hasIncDecPlus && (!parent->astParent() || parent->astParent()->str() == MatchCompiler::makeConstString(";")))
                continue;
            if (match68(parent) && match87(parent->astOperand1()))
                continue;
            if (match31(parent)) {
...
{
    if (!match59(tok) || !tok->astOperand1())
        return false;
    tok = tok->astOperand1();
    if (tok->str() == MatchCompiler::makeConstString("."))
        tok = tok->astOperand2();
...
static bool isConstStatement(const Token *tok, const Library& library, bool isNestedBracket = false)
{
    ...
    if (match98(tok) && (tok->astOperand1() || tok->astOperand2()))
        return true;
    if (match99(tok->previous()) && tok->previous()->isKeyword())
        return true;
    if (isCPPCast(tok)) {
    ...
    if (match104(tok2)) {
        ...
        while (tok2->index() > 1 && match105(tok2->tokAt(-2))) {
...
static bool isVoidStmt(const Token *tok)
{
    if (match106(tok))
        return true;
    if (isCPPCast(tok) && tok->astOperand1() && match107(tok->astOperand1()->next()))
        return true;
    const Token *tok2 = tok;
    while (tok2->astOperand1())
        tok2 = tok2->astOperand1();
    if (match108(tok2->previous()) && match106(tok2->linkAt(-1)))
        return true;
    if (match106(tok2))
        return true;
    return match109(tok2->previous());
}
...
static bool isConstTop(const Token *tok)
{
    ...
    if (match51(tok->astParent()) &&
        match110(tok->astTop()->previous()) && match51(tok->astTop()->astOperand2())) {
    ...
    if (match59(tok)) {
        ...
        while (match59(bracTok->astParent()))
...
void CheckOther::checkIncompleteStatement()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (!isConstTop(tok))
            continue;
        if (tok->str() == MatchCompiler::makeConstString(",") && match62(tok->astTop()->previous()))
            continue;
        ...
        if (!match51(tok->astParent()) && !match51(rtok) &&
            !match112(tok->previous()) &&
            !(tok->isCpp() && tok->isCast() && !tok->astParent()) &&
            !(!tok->astParent() && tok->astOperand1() && match32(tok->previous()) && tok->previous()->isKeyword()) &&
            !match62(tok->tokAt(-2)) &&
            !match89(tok->tokAt(-1)) &&
...
void CheckOther::constStatementError(const Token *tok, const std::string &type, bool inconclusive)
{
    ...
    else if (match117(tok))
        msg = "Redundant code: Found unused function.";
    else if (match32(tok->previous()))
...
void CheckOther::checkNanInArithmeticExpression()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        if (tok->str() != MatchCompiler::makeConstString("/"))
            continue;
        if (!match118(tok->astParent()))
            continue;
        if (match119(tok->astOperand2()))
...
void CheckOther::checkMisusedScopedObject()
{
    ...
    auto getConstructorTok = [](const Token* tok, std::string& typeStr) -> const Token* {
        if (!match120(tok) || tok->next()->isKeyword())
            return nullptr;
        tok = tok->next();
        typeStr.clear();
        while (match105(tok)) {
        ...
        if (match122(endTok) && match123(endTok->linkAt(1)) &&
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token *tok = scope->bodyStart; tok && tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (tok->isAssignmentOp() && match68(tok->astOperand1()) && tok->astOperand1()->astOperand1()) {
                if (const Function* ftok = tok->astOperand1()->astOperand1()->function()) {
                    if (ftok->retType && match124(ftok->retType->classDef) && !Function::returnsReference(ftok, /*unknown*/ false, /*includeRValueRef*/ true))
...
void CheckOther::checkInvalidFree()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok && tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if ((tok->isCpp() && match127(tok)) ||
                (match128(tok) && mSettings->library.getAllocFuncInfo(tok->tokAt(2)))) {
            ...
            else if ((match131(tok) && mSettings->library.getDeallocFuncInfo(tok)) ||
                     match132(tok) ||
                     match133(tok)) {
            ...
            // if it is later used to free memory
                ...
                const Token* tok2 = findmatch134(tok->next(), tok->linkAt(1)) ;
                while (tok2 != nullptr) {
                    allocation.erase(tok->varId());
                    inconclusive.erase(tok2->varId());
                    tok2 = findmatch134(tok2->next(), tok->linkAt(1)) ;
                }
            }
        }
    }
    ...
    for (const Scope *scope : symbolDatabase->functionScopes) {
        for (const Token *tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            ...
            auto isInsideLambdaCaptureList = [](const Token* tok) {
                const Token* parent = tok->astParent();
                while (match66(parent))
                    parent = parent->astParent();
                return isLambdaCaptureList(parent);
            };
            ErrorPath errorPath;
            if (tok->isOp() && tok->astOperand1() && !match141(tok) && !isInsideLambdaCaptureList(tok)) {
                if (match142(tok) && astIsFloat(tok->astOperand1(), true))
                    continue;
                const bool pointerDereference = (tok->astOperand1() && tok->astOperand1()->isUnaryOp("*")) ||
                                                (tok->astOperand2() && tok->astOperand2()->isUnaryOp("*"));
                const bool followVar = (!isConstVarExpression(tok) || match143(tok)) && !pointerDereference;
                ...
                } else if (!match145(tok)) { // These operators are not associative
...
void CheckOther::duplicateExpressionError(const Token *tok1, const Token *tok2, const Token *opTok, ErrorPath errors, bool hasMultipleExpr)
{
    ...
    if (expr1 != expr2 && (!opTok || match146(opTok))) {
        id = "knownConditionTrueFalse";
        std::string exprMsg = "The comparison \'" + expr1 + " " + op +  " " + expr2 + "\' is always ";
        if (match147(opTok))
            msg = exprMsg + "true";
        else if (match148(opTok))
            msg = exprMsg + "false";
    }

    if (expr1 != expr2 && !match149(tok1) && !match149(tok2))
...
bool CheckOther::testIfNonZeroExpressionIsPositive(const Token *tok, const ValueFlow::Value *&zeroValue, const Token *&nonZeroExpr)
{
    ...
    if (match153(tok) && v2 && v2->isKnown()) {
        zeroValue = v2;
        nonZeroExpr = tok->astOperand1();
    } else if (match154(tok) && v1 && v1->isKnown()) {
        zeroValue = v1;
        nonZeroExpr = tok->astOperand2();
    } else {
        return false;
...
void CheckOther::checkRedundantCopy()
{
    ...
    for (const Variable* var : symbolDatabase->variableList()) {
        ...
        if (startTok->strAt(1) == MatchCompiler::makeConstString("=")) // %type% %name% = ... ;
            ;
        else if (match155(startTok->next()) && var->isClass()) {
        ...
        if (!match32(tok->previous()))
            continue;
        if (!match156(tok->link())) // bailout for usage like "const A a = getA()+3"
            continue;
        ...
        if (func && func->tokenDef->strAt(-1) == MatchCompiler::makeConstString("&")) {
            ...
            if (fScope && fScope->bodyEnd && match157(fScope->bodyEnd->tokAt(-3))) {
...
void CheckOther::checkInterlockedDecrement()
{
    ...
    for (const Token *tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        if (tok->isName() && match164(tok)) {
            const Token* interlockedVarTok = tok->tokAt(3);
            const Token* checkStartTok =  interlockedVarTok->tokAt(5);
            if ((match165(checkStartTok) && checkStartTok->strAt(2) == interlockedVarTok->str()) ||
                (match166(checkStartTok) && checkStartTok->strAt(1) == interlockedVarTok->str()) ||
                (match167(checkStartTok) && checkStartTok->str() == interlockedVarTok->str()) ||
                (match168(checkStartTok) && checkStartTok->str() == interlockedVarTok->str())) {
                raceAfterInterlockedDecrementError(checkStartTok);
            }
        } else if (match169(tok)) {
...
void CheckOther::checkUnusedLabel()
{
    ...
    for (const Scope * scope : symbolDatabase->functionScopes) {
        ...
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (match172(tok) && !tok->tokAt(1)->isKeyword()) {
...
static bool checkEvaluationOrderC(const Token * tok, const Token * tok2, const Token * parent, const Settings & settings, bool & selfAssignmentError)
{
    ...
    visitAstNodes((parent->astOperand1() != tok2) ? parent->astOperand1() : parent->astOperand2(), [&](const Token *tok3) {
        ...
        if (tok3->str() == MatchCompiler::makeConstString("(") && match173(tok3->previous()))
...
static bool isVariableExprHidden(const Token* tok)
{
    ...
    if (!tok->astParent())
        return false;
    if (match88(tok->astParent()) && match176(tok->astSibling()))
        return true;
    if (match177(tok->astParent()) && match178(tok->astSibling()))
        return true;
    if (match179(tok->astParent()) && match180(tok->astSibling()))
        return true;
    return false;
}
...
void CheckOther::checkKnownArgument()
{
    ...
    for (const Scope *functionScope : symbolDatabase->functionScopes) {
        for (const Token *tok = functionScope->bodyStart; tok != functionScope->bodyEnd; tok = tok->next()) {
            ...
            if (match181(tok))
                continue;
            if (!match182(tok->astParent()))
                continue;
            if (tok->astParent()->isCast() || (tok->isCast() && match181(tok->astOperand2())))
                continue;
            int argn = -1;
...
void CheckOther::checkKnownPointerToBool()
{
    ...
    for (const Scope* functionScope : symbolDatabase->functionScopes) {
        for (const Token* tok = functionScope->bodyStart; tok != functionScope->bodyEnd; tok = tok->next()) {
            ...
            if (!astIsPointer(tok))
                continue;
            if (match185(tok->astParent()))
                continue;
            if (tok->astParent() && match186(tok->astParent()->previous()))
                continue;
            if (tok->isExpandedMacro())
                continue;
...
static bool getBufAndOffset(const Token *expr, const Token *&buf, MathLib::bigint *offset, const Settings& settings, MathLib::bigint* sizeValue = nullptr)
{
    ...
    if (expr->isUnaryOp("&") && match59(expr->astOperand1())) {
    ...
    } else if (match190(expr) && expr->isBinaryOp()) {
