...
#include <string>
#include <cstring>
#line 1 "/opt/cppcheck/cppcheck-main/lib/infer.cpp"
/*
                                    ...
                                    std::list<ValueFlow::Value> rhsValues)
{
    ...
    if (op == MatchCompiler::makeConstString("-")) {
    ...
    } else if ((op == MatchCompiler::makeConstString("!=") || op == MatchCompiler::makeConstString("==")) && lhs.isScalarOrEmpty() && rhs.isScalarOrEmpty()) {
        if (lhs.isScalar() && rhs.isScalar()) {
            ...
            if (!refs.empty()) {
                ValueFlow::Value value(op == MatchCompiler::makeConstString("!="));
                addToErrorPath(value, refs);
                setValueKind(value, refs);
                result.push_back(std::move(value));
            }
