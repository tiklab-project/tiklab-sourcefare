...
MAYBE_UNUSED static inline bool match1(const Token* tok) {
    if (!tok || !(tok->str() == MatchCompiler::makeConstString("va_start")))
        return false;
    tok = tok->next();
...
// pattern: va_start ( %varid%
MAYBE_UNUSED static inline bool match2(const Token* tok, const int varid) {
    if (!tok || !(tok->str() == MatchCompiler::makeConstString("va_start")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")))
...
// pattern: va_end ( %varid%
MAYBE_UNUSED static inline bool match3(const Token* tok, const int varid) {
    if (!tok || !(tok->str() == MatchCompiler::makeConstString("va_end")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")))
        return false;
...
MAYBE_UNUSED static inline bool match4(const Token* tok) {
    if (!tok || !(tok->str() == MatchCompiler::makeConstString("va_copy")))
        return false;
    tok = tok->next();
    if (!tok || !((tok->tokType() == Token::eExtendedOp) && tok->str() == MatchCompiler::makeConstString("(")))
        return false;
...
#line 1 "/opt/cppcheck/cppcheck-main/lib/checkvaarg.cpp"
/*
...
void CheckVaarg::va_list_usage()
{
    ...
    for (const Variable* var : symbolDatabase->variableList()) {
        if (!var || var->isPointer() || var->isReference() || var->isArray() || !var->scope() || var->typeStartToken()->str() != MatchCompiler::makeConstString("va_list"))
            continue;
        ...
        for (; tok && tok != var->scope()->bodyEnd; tok = tok->next()) {
            ...
            if (tok2)
                tok = tok2;
            if (match2(tok, var->declarationId())) {
                if (open)
                    va_start_subsequentCallsError(tok, var->name());
                open = true;
                tok = tok->linkAt(1);
            } else if (match3(tok, var->declarationId())) {
                if (!open)
                ...
                if (!tok)
                    return;
            } else if (tok->str() == MatchCompiler::makeConstString("goto") || (tok->isCpp() && tok->str() == MatchCompiler::makeConstString("try"))) {
                open = false;
                break;
            } else if (!open && tok->varId() == var->declarationId())
                va_list_usedBeforeStartedError(tok, var->name());
            else if (exitOnEndOfStatement && tok->str() == MatchCompiler::makeConstString(";"))
                break;
        }
