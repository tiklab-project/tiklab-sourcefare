...
#include <string>
#include <cstring>
#line 1 "/opt/cppcheck/cppcheck-main/lib/settings.cpp"
/*
...
std::string Settings::parseEnabled(const std::string &str, std::tuple<SimpleEnableGroup<Severity>, SimpleEnableGroup<Checks>> &groups)
{
    ...
    if (str == MatchCompiler::makeConstString("all")) {
    ...
    } else if (str == MatchCompiler::makeConstString("warning")) {
        severity.enable(Severity::warning);
    } else if (str == MatchCompiler::makeConstString("style")) {
        severity.enable(Severity::style);
    } else if (str == MatchCompiler::makeConstString("performance")) {
        severity.enable(Severity::performance);
    } else if (str == MatchCompiler::makeConstString("portability")) {
        severity.enable(Severity::portability);
    } else if (str == MatchCompiler::makeConstString("information")) {
        severity.enable(Severity::information);
    } else if (str == MatchCompiler::makeConstString("unusedFunction")) {
        checks.enable(Checks::unusedFunction);
    } else if (str == MatchCompiler::makeConstString("missingInclude")) {
        checks.enable(Checks::missingInclude);
    }
#ifdef CHECK_INTERNAL
    else if (str == MatchCompiler::makeConstString("internal")) {
