...
MAYBE_UNUSED static inline bool match8(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("*")) || ((tok->tokType() == Token::eExtendedOp || tok->tokType() == Token::eLambda) && tok->str() == MatchCompiler::makeConstString("[")) || (tok->str() == MatchCompiler::makeConstString(".")) || ((tok->tokType() == Token::eIncDecOp) && tok->str() == MatchCompiler::makeConstString("++")) || ((tok->tokType() == Token::eIncDecOp) && tok->str() == MatchCompiler::makeConstString("--"))))
        return false;
    return true;
}
// pattern: =
...
MAYBE_UNUSED static inline bool match12(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eAssignmentOp) && tok->str() == MatchCompiler::makeConstString("+=")) || ((tok->tokType() == Token::eAssignmentOp) && tok->str() == MatchCompiler::makeConstString("-=")) || ((tok->tokType() == Token::eAssignmentOp) && tok->str() == MatchCompiler::makeConstString("*=")) || ((tok->tokType() == Token::eIncDecOp) && tok->str() == MatchCompiler::makeConstString("++")) || ((tok->tokType() == Token::eIncDecOp) && tok->str() == MatchCompiler::makeConstString("--"))))
        return false;
    return true;
}
// pattern: ++|--|+=
MAYBE_UNUSED static inline bool match13(const Token* tok) {
    if (!tok || !(((tok->tokType() == Token::eIncDecOp) && tok->str() == MatchCompiler::makeConstString("++")) || ((tok->tokType() == Token::eIncDecOp) && tok->str() == MatchCompiler::makeConstString("--")) || ((tok->tokType() == Token::eAssignmentOp) && tok->str() == MatchCompiler::makeConstString("+="))))
        return false;
    return true;
}
// pattern: ++|--
...
MAYBE_UNUSED static inline bool match25(const Token* tok) {
    if (!tok || !((tok->str() == MatchCompiler::makeConstString(".")) || ((tok->tokType() == Token::eBitOp) && tok->str() == MatchCompiler::makeConstString("&")) || ((tok->tokType() == Token::eArithmeticalOp) && tok->str() == MatchCompiler::makeConstString("*")) || (tok->varId() != 0)))
        return false;
    return true;
}
// pattern: %oror%|&&
...
#line 1 "/opt/cppcheck/cppcheck-main/lib/vf_analyzers.cpp"
/*
...
static bool isDereferenceOp(const Token* tok)
{
    ...
    if (tok->str() == MatchCompiler::makeConstString("*"))
        return true;
    return tok->str() == MatchCompiler::makeConstString(".") && tok->originalName() == MatchCompiler::makeConstString("->");
}
...
struct ValueFlowAnalyzer : Analyzer {
    ...
    static ValueFlow::Value::MoveKind isMoveOrForward(const Token* tok)
    {
        ...
        if (!ftok)
            return ValueFlow::Value::MoveKind::NonMovedVariable;
        if (match6(ftok->astOperand1()))
            return ValueFlow::Value::MoveKind::MovedVariable;
        if (match7(ftok->astOperand1()))
    ...
    virtual Action isModified(const Token* tok) const {
        ...
        if (isVariableChanged(tok, getIndirect(tok), getSettings())) {
            if (match8(tok->astParent()))
                return Action::Read | Action::Invalid;
            // Check if its assigned to the same value
            if (value && !value->isImpossible() && match9(tok->astParent()) && astIsLHS(tok) &&
    ...
    virtual Action isWritable(const Token* tok, Direction d) const {
        ...
        if (value->isImpossible() && !match12(parent))
            return Action::None;
    ...
    virtual void writeValue(ValueFlow::Value* value, const Token* tok, Direction d) const {
        ...
        if (tok->astParent()->isAssignmentOp()) {
            ...
            bool inc = tok->astParent()->str() == MatchCompiler::makeConstString("++");
...
private:
    ...
    Action analyzeLifetime(const Token* tok) const
    {
        ...
        if (match(tok))
            return Action::Match;
        if (match15(tok) && analyzeLifetime(tok->astOperand1()) != Action::None)
            return Action::Read;
        if (astIsRHS(tok) && match15(tok->astParent()))
    ...
    Action isGlobalModified(const Token* tok) const
    {
        if (tok->function()) {
            ...
            return Action::None;
        } else if (match15(tok->astParent()) && astIsContainer(tok->astParent()->astOperand1())) {
            ...
            return Action::None;
        } else if (!tok->isKeyword() && match3(tok)) {
            return Action::Invalid;
        }
        return Action::None;
    }
    ...
    static bool evalAssignment(Value& lhsValue, const std::string& assign, const ValueFlow::Value& rhsValue)
    {
        ...
        if (lhsValue.isSymbolicValue() && rhsValue.isIntValue()) {
            if (assign != MatchCompiler::makeConstString("+=") && assign != MatchCompiler::makeConstString("-="))
                return false;
    ...
    Action analyzeMatch(const Token* tok, Direction d) const {
        const Token* parent = tok->astParent();
        if (d == Direction::Reverse && isGlobal() && !dependsOnThis() && match4(parent)) {
            Action a = isGlobalModified(parent->next());
            if (a != Action::None)
                return a;
        }
        if ((astIsPointer(tok) || astIsSmartPointer(tok)) &&
            (match17(parent) || (parent && parent->originalName() == MatchCompiler::makeConstString("->"))) && getIndirect(tok) <= 0)
            return Action::Read;
        ...
        if (match(ref)) {
            ...
            if (la.matches()) {
                ...
                if (isModified(tok).isModified())
                    a = Action::Invalid;
                if (match16(tok->astParent()) && astIsLHS(tok))
                    a |= Action::Invalid;
    ...
    Action analyze(const Token* tok, Direction d) const override {
        ...
        if (isGlobal() && !dependsOnThis() && match3(tok) && !tok->variable() &&
            !match18(tok->linkAt(1))) {
    ...
    void assume(const Token* tok, bool state, unsigned int flags) override {
        ...
        const Token* parent = tok->astParent();
        if (parent) {
            isCondBlock = match20(parent->previous());
        }

        if (isCondBlock) {
            const Token* startBlock = parent->link()->next();
            if (match21(startBlock) && match22(parent->tokAt(-2)))
...
static bool bifurcate(const Token* tok, const std::set<nonneg int>& varids, const Settings& settings, int depth)
{
    ...
    if (tok->varId() != 0) {
        ...
        if (!start)
            return false;
        if (start->strAt(-1) == MatchCompiler::makeConstString(")") || start->strAt(-1) == MatchCompiler::makeConstString("}"))
            return false;
        if (match24(start, var->declarationId()))
            start = start->tokAt(2);
    ...
    bool stopOnCondition(const Token* condTok) const override {
        ...
        if (!condTok->hasKnownIntValue() && values.count(condTok->varId()) == 0) {
            ...
            return std::any_of(values_.cbegin(), values_.cend(), [](const ValueFlow::Value& v) {
                return v.isSymbolicValue() && match26(v.tokvalue);
            });
        }
        return false;
    }
...
struct ExpressionAnalyzer : SingleValueFlowAnalyzer {
        ...
        expr(e)
    {
        ...
        uniqueExprId =
            expr->isUniqueExprId() && (match1(expr) || !isVariableChanged(expr, 0, s));
    }
        ...
        visitAstNodes(start, [&](const Token* tok) {
            ...
            if (tok->varId() > 0) {
                ...
                if (!match15(tok->previous())) {
                    const Variable* var = tok->variable();
                    if (var && var->isReference() && var->isLocal() && match27(var->nameToken()) &&
                        !isGlobalData(var->nameToken()->next()->astOperand2()))
                        return ChildrenToVisit::none;
                    const bool deref = tok->astParent() &&
                                       (tok->astParent()->isUnaryOp("*") ||
                                        (tok->astParent()->str() == MatchCompiler::makeConstString("[") && tok == tok->astParent()->astOperand1()));
...
struct ContainerExpressionAnalyzer : ExpressionAnalyzer {
    ...
    Action isWritable(const Token* tok, Direction /*d*/) const override
    {
        ...
        if (container->stdStringLike && match29(parent) && astIsLHS(tok) && parent->astOperand2()) {
            const Token* rhs = parent->astOperand2();
        ...
        } else if (astIsLHS(tok) && match4(tok->astParent())) {
    ...
    void writeValue(ValueFlow::Value* val, const Token* tok, Direction d) const override {
        ...
        if (container->stdStringLike && match29(parent) && parent->astOperand2()) {
            const Token* rhs = parent->astOperand2();
            const Library::Container* rhsContainer = getLibraryContainer(rhs);
            if (rhs->tokType() == Token::eString)
