...
MAYBE_UNUSED static inline bool match1(const Token* tok) {
    if (!tok || !(tok->str() == MatchCompiler::makeConstString("assert")))
        return false;
    tok = tok->next();
...
#line 1 "/opt/cppcheck/cppcheck-main/lib/checkassert.cpp"
/*
    ...
    for (const Token* tok = mTokenizer->list.front(); tok; tok = tok->next()) {
        ...
        for (const Token* tmp = tok->next(); tmp != endTok; tmp = tmp->next()) {
            ...
            if (tmp->tokType() != Token::eFunction) {
                if (const Library::Function* f = mSettings->library.getFunction(tmp)) {
                    ...
                    if (std::any_of(f->argumentChecks.begin(), f->argumentChecks.end(), [](const std::pair<int, Library::ArgumentChecks>& ac) {
                        ...
                        continue;
                    if (tmp->str() == MatchCompiler::makeConstString("get") && match3(tmp->astParent()) && astIsSmartPointer(tmp->astParent()->astOperand1()))
                        continue;
            ...
            for (const Token *tok2 = scope->bodyStart; tok2 != scope->bodyEnd; tok2 = tok2->next()) {
                ...
                if (var->isArgument() && var->isPointer() && tok2->strAt(-2) != MatchCompiler::makeConstString("*"))
                ...
                for (const Token *rt = scope->bodyStart; rt != scope->bodyEnd; rt = rt->next()) {
                    if (rt->str() != MatchCompiler::makeConstString("return")) continue; // find all return statements
