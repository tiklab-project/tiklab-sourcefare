...
#include <string>
#include <cstring>
#line 1 "/opt/cppcheck/cppcheck-main/lib/importproject.cpp"
/*
...
bool ImportProject::importCppcheckGuiProject(std::istream &istr, Settings &settings, Suppressions &supprs, bool premium)
{
    ...
    for (const tinyxml2::XMLElement *node = rootnode->FirstChildElement(); node; node = node->NextSiblingElement()) {
        ...
        if (strcmp(name, CppcheckXml::RootPathName) == 0) {
            ...
            temp.analyzeAllVsConfigs = std::string(empty_if_null(node->GetText())) != MatchCompiler::makeConstString("false");
