...
#include <string>
#include <cstring>
#line 1 "/opt/cppcheck/cppcheck-main/lib/preprocessor.cpp"
/*
...
static void addInlineSuppressions(const simplecpp::TokenList &tokens, const Settings &settings, SuppressionList &suppressions, std::list<BadInlineSuppression> &bad)
{
    ...
    for (const simplecpp::Token *tok = tokens.cfront(); tok; tok = tok->next) {
        ...
        if (tok->str() == MatchCompiler::makeConstString("#") && tok->next && tok->next->str() == MatchCompiler::makeConstString("define")) {
    ...
    for (const simplecpp::TokenList *tokenList : list) {
        for (const simplecpp::Token *tok = tokenList->cfront(); tok; tok = tok->next) {
            if ((tok->op != '#') || (tok->previous && tok->previous->location.line == tok->location.line))
                continue;
            if (tok->next && tok->next->str() == MatchCompiler::makeConstString("endfile"))
                continue;
            ...
            for (const simplecpp::Token *tok2 = tok; tok2 && tok2->location.line == directive.linenr; tok2 = tok2->next) {
                ...
                if (!directive.str.empty() && (tok2->location.col > tok2->previous->location.col + tok2->previous->str().size()))
                    directive.str += ' ';
                if (directive.str == MatchCompiler::makeConstString("#") && tok2->str() == MatchCompiler::makeConstString("file"))
...
static std::string readcondition(const simplecpp::Token *iftok, const std::set<std::string> &defined, const std::set<std::string> &undefined)
{
    ...
    if (len == 1 && cond->str() == MatchCompiler::makeConstString("0"))
        return "0";
    ...
    if (len == 3 && cond->name && next1->str() == MatchCompiler::makeConstString("==") && next2->number) {
        if (defined.find(cond->str()) == defined.end())
    ...
    for (; sameline(iftok,cond); cond = cond->next) {
        if (cond->op == '!') {
            if (!sameline(iftok,cond->next) || !cond->next->name)
                break;
            if (cond->next->str() == MatchCompiler::makeConstString("defined"))
                continue;
            configset.insert(cond->next->str() + "=0");
            continue;
        }
        if (cond->str() != MatchCompiler::makeConstString("defined"))
            continue;
...
static std::string cfg(const std::vector<std::string> &configs, const std::string &userDefines)
{
    ...
    for (const std::string &c : configs2) {
        if (c.empty())
            continue;
        if (c == MatchCompiler::makeConstString("0"))
            return "";
...
static bool isUndefined(const std::string &cfg, const std::set<std::string> &undefined)
{
    for (std::string::size_type pos1 = 0U; pos1 < cfg.size();) {
        ...
        if (eq == std::string::npos && undefined.find(def) != undefined.end())
            return true;
        if (eq != std::string::npos && undefined.find(def.substr(0,eq)) != undefined.end() && def.substr(eq) != MatchCompiler::makeConstString("=0"))
            return true;
    ...
    while (nullptr != (cmdtok = cmdtok->next)) {
        if (cmdtok->op == '#' && !sameline(cmdtok->previous,cmdtok) && sameline(cmdtok, cmdtok->next)) {
            if (startsWith(cmdtok->next->str(),"if"))
                ++level;
            else if (cmdtok->next->str() == MatchCompiler::makeConstString("endif")) {
                --level;
                if (level < 0)
                    return cmdtok;
            }
...
static void getConfigs(const simplecpp::TokenList &tokens, std::set<std::string> &defined, const std::string &userDefines, const std::set<std::string> &undefined, std::set<std::string> &ret)
{
    ...
    for (const simplecpp::Token *tok = tokens.cfront(); tok; tok = tok->next) {
        ...
        if (!sameline(tok, cmdtok))
            continue;
        if (cmdtok->str() == MatchCompiler::makeConstString("ifdef") || cmdtok->str() == MatchCompiler::makeConstString("ifndef") || cmdtok->str() == MatchCompiler::makeConstString("if")) {
            std::string config;
            if (cmdtok->str() == MatchCompiler::makeConstString("ifdef") || cmdtok->str() == MatchCompiler::makeConstString("ifndef")) {
                ...
                if (defined.find(config) != defined.end())
                    config.clear();
            } else if (cmdtok->str() == MatchCompiler::makeConstString("if")) {
            ...
            bool ifndef = false;
            if (cmdtok->str() == MatchCompiler::makeConstString("ifndef"))
                ifndef = true;
            else {
            ...
            configs_if.push_back((cmdtok->str() == MatchCompiler::makeConstString("ifndef")) ? std::string() : config);
            configs_ifndef.push_back((cmdtok->str() == MatchCompiler::makeConstString("ifndef")) ? std::move(config) : std::string());
            ret.insert(cfg(configs_if,userDefines));
        } else if (cmdtok->str() == MatchCompiler::makeConstString("elif") || cmdtok->str() == MatchCompiler::makeConstString("else")) {
            ...
            if (cmdtok->str() == MatchCompiler::makeConstString("else") &&
                cmdtok->next &&
                !sameline(cmdtok,cmdtok->next) &&
                sameline(cmdtok->next, cmdtok->next->next) &&
                cmdtok->next->op == '#' &&
                cmdtok->next->next->str() == MatchCompiler::makeConstString("error")) {
            ...
            if (!configs_if.empty())
                configs_if.pop_back();
            if (cmdtok->str() == MatchCompiler::makeConstString("elif")) {
        ...
        } else if (cmdtok->str() == MatchCompiler::makeConstString("endif") && !sameline(tok, cmdtok->next)) {
            if (!configs_if.empty())
                configs_if.pop_back();
            if (!configs_ifndef.empty())
                configs_ifndef.pop_back();
        } else if (cmdtok->str() == MatchCompiler::makeConstString("error")) {
        ...
        } else if (cmdtok->str() == MatchCompiler::makeConstString("define") && sameline(tok, cmdtok->next) && cmdtok->next->name) {
...
void Preprocessor::simplifyPragmaAsmPrivate(simplecpp::TokenList &tokenList)
{
    ...
    for (simplecpp::Token *tok = tokenList.front(); tok; tok = tok->next) {
        ...
        if (!tok2 || !sameline(tok, tok2) || tok2->str() != MatchCompiler::makeConstString("pragma"))
            continue;

        const simplecpp::Token * const tok3 = tok2->nextSkipComments();
        if (!tok3 || !sameline(tok, tok3) || tok3->str() != MatchCompiler::makeConstString("asm"))
            continue;
        ...
        while ((endasm = endasm->next) != nullptr) {
            ...
            if (!endasm2 || !sameline(endasm, endasm2) || endasm2->str() != MatchCompiler::makeConstString("pragma"))
                continue;
            const simplecpp::Token * const endasm3 = endasm2->nextSkipComments();
            if (!endasm3 || !sameline(endasm2, endasm3) || endasm3->str() != MatchCompiler::makeConstString("endasm"))
                continue;
