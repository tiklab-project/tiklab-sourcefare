...
rem        wmic process where "ProcessId=%PGSQL_PID%" get ProcessId | findstr "%PGSQL_PID%" >nul
rem         if %errorlevel%==0 (
rem            echo PGSQL ALREADY STARED (PID=%PGSQL_PID%^)
rem         ) else (
rem             echo PGSQL IS NOT RUNNING......
rem         )
rem     ) else (
rem         echo PGSQL IS NOT RUNNING......
rem     )
